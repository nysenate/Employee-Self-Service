 ...
 */
function(){"use strict";function floatThead($timeout,$log){function link(scope,element,attrs,ngModel){$(element).floatThead(scope.$eval(attrs.floatThead)),ngModel?scope.$watch(attrs.ngModel,function(){$(element).floatThead("reflow")},!0):$log.info("floatThead: ngModel not provided!"),element.bind("update",function(){$timeout(function(){$(element).floatThead("reflow")},0)}),element.bind("$destroy",function(){$(element).floatThead("destroy")})}var directive={require:"?ngModel",link:link,restrict:"A"};return directive}angular.module("floatThead",[]).directive("floatThead",["$timeout","$log",floatThead])}();