...
export default function AlertInfoForm({ alertInfo }) {
  ...
  const onSubmit = (data) => {
    ...
    if (duplicatePhoneFields.length > 0) {
      duplicatePhoneFields.forEach((field) =>
        setError(field, {
          type: "custom",
          message: "Remove duplicate phone numbers",
        }),
      );
    }
    ...
    if (duplicateEmailFields.length > 0) {
      duplicateEmailFields.forEach((field) =>
        setError(field, {
          type: "custom",
...
function formatPhoneNumber(phoneNumberString) {
  const cleanNumber = (phoneNumberString ?? "").replaceAll(/\D/gi, "");
...
const duplicatePhoneNumbers = (formData) => {
  ...
  const phoneNumbers = phoneNumberFields
    .map((f) => formatPhoneNumber(formData[f]))
    .filter((f) => f !== ""); // don't check empty phone numbers for duplicates.
  const duplicateNumbers = findDuplicates(phoneNumbers);
  return phoneNumberFields.filter((field) =>
    duplicateNumbers.includes(formatPhoneNumber(formData[field])),
  );
};
...
const duplicateEmails = (formData) => {
  ...
  const emailAddresses = emailFields
    .map((e) => cleanEmail(formData[e] ?? ""))
    .filter((e) => e !== ""); // don't check empty emails for duplicates.
  const duplicateEmailAddresses = findDuplicates(emailAddresses);
  return emailFields.filter((field) =>
    duplicateEmailAddresses.includes(cleanEmail(formData[field])),
  );
};
