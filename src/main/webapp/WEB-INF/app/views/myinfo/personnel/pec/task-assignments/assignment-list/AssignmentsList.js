          ...
          incompleteAssignments.map((assignment) => (
            ...
            >
              ...
              {assignment.task.taskType === "EVERFI_COURSE" ? (
                ...
                >
                  <u>
                    {assignmentDisplayFields(assignment.task.taskType).verb}:{" "}
      ...
      <ul className={"my-2"}>
        {completedAssignments.length === 0 ? (
          ...
          completedAssignments.map((assignment) => (
            <li
              className={"ml-8 box-border flex p-1 text-[12.5px]"}
              key={assignment.task.taskId}
            >
              <CheckIcon className={"size-4 text-teal-600"} />
              {assignment.task.taskType === "EVERFI_COURSE" ? (
                <a
                  className="ml-1 font-normal text-teal-600 transition-opacity duration-300 hover:bg-gray-50"
                  href={assignment.task.url}
                  target="_blank"
                  rel="noopener noreferrer"
                >
                  ...
                  <span className={"ml-1 text-gray-400"}>
                    -{" "}
                    {
                      assignmentDisplayFields(assignment.task.taskType)
                        .verbPastTense
                    }{" "}
                    on {isoToMediumDate(assignment.timestamp)}
                  </span>
                </a>
              ) : (
                <Link
                  ...
                  className="ml-1 font-normal text-teal-600 hover:bg-gray-50"
                >
                  <u>{assignment.task.title}</u>
                  <span className={"ml-1 text-gray-400"}>
                    -{" "}
