...
export default function OfficeMultiSelect({ onChange }) {
  ...
  const removeSelectedOffice = office => {
    setSelectedOffices((selectedOffices) => selectedOffices.filter(o => o.code !== office.code))
  }
      ...
      <Combobox multiple
        ...
        <ComboboxOptions anchor="top"
          ...
          {!respctrHeadSearchQuery.isLoading
            && respctrHeadSearchQuery.data.filter(o => !selectedOffices.includes(o)).map((office) => (
              <div key={office.code}>
                <hr/>
