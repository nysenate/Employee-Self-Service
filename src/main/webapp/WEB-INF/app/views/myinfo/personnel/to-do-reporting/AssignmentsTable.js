...
import { useTasks } from "app/api/tasksApi";
import { Button } from "app/components/Button";
import useAuth from "app/contexts/Auth/useAuth";
import Modal from "app/components/Modal";
import { useManuallyDeactivateTaskAssignment, useManuallyOverrideCompletionStatus } from "app/api/taskAssignmentApi";
    ...
    <>
      ...
      {showDetails &&
        <tr className={showDetailsClasses}>
          <td colSpan="3">
            <AssignmentRowDetails emp={emp} assignments={assignments} tasksMap={tasksMap}/>
          </td>
        </tr>
      }
    </>
        ...
        <div className="my-2">
          ...
          <ul className="list-disc ml-8">
            {incompleteAssignments.map(assignment => (
              <IncompleteAssignmentDetails key={assignment.taskId} emp={emp} assignment={assignment}
                                           tasksMap={tasksMap}/>
            ))}
          </ul>
        </div>
              ...
              <li key={assignment.taskId}>
                ...
                <div className="mt-1 mb-4">
                  <a
                    ...
                    target="_blank" rel="noopener noreferrer">Download signed pdf</a>
                  <span className="font-light text-gray-700"> completed {isoToMediumDate(assignment.timestamp)}</span>
                </div>
              </li>
            ))}
          </ul>
...
function IncompleteAssignmentDetails({ emp, assignment, tasksMap }) {
  ...
  return (
    <li>
      ...
      <ManuallyOverrideModal isOpen={isOverrideModalOpen}
                             setIsOpen={setIsOverrideModalOpen}
                             emp={emp}
                             assignment={assignment}
                             task={tasksMap.get(assignment.taskId)}/>
      <ManuallyDeactivateModal isOpen={isDeactivateModalOpen}
...
function ManuallyOverrideModal({ isOpen, setIsOpen, emp, assignment, task }) {
  ...
  const onProceed = () => {
    manuallyOverrideApi.mutateAsync({
    ...
    <Modal isOpen={isOpen}>
      ...
      <Modal.Body>
        <div className="text-center">
          Warning: You are attempting to submit a task COMPLETION override for employee<br/>
          {emp.fullName}<br/>
          for task<br/>
          {task.title}
        </div>
...
function ManuallyDeactivateModal({ isOpen, setIsOpen, emp, assignment, task }) {
  ...
  const onProceed = () => {
    deactivateTaskAssignmentApi.mutateAsync({
      updatedByEmpId: userEmpId,
      taskId: task.taskId,
      isActive: false,
      assignedEmpId: assignment.empId
  ...
  return (
    <Modal isOpen={isOpen}>
      ...
      <Modal.Body>
        <div className="text-center">
          Warning: You are attempting to submit a task ACTIVE STATUS override for employee<br/>
          {emp.fullName}<br/>
          for task<br/>
          {task.title}
        </div>
