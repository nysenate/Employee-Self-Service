...
const Calendar = () => {
  ...
  const periodHighlight = (date) => {
    ...
    if (day === 6 || day === 0) {
      ...
      if (periodMap[mDateStr] && !periodMap[mDateStr].endYearSplit) {
        toolTips.push(
          `Last Day of Pay Period ${periodMap[mDateStr]["payPeriodNum"]}`,
        );
  ...
  return (
    <div>
      ...
      <div
        className={`${styles.contentContainer} ${styles.payPeriodCalContainer}`}
      >
        <div className={`${styles.contentInfo} ${styles.legendContainer}`}>
          <div
            className={`${styles.legendBlock} ${styles.payPeriodCalPayPeriodEnd}`}
          >
            {"\u00A0"}
          </div>
          Pay Period End Date
          <div
            className={`${styles.legendBlock} ${styles.payPeriodCalHoliday}`}
          >
            {"\u00A0"}
          </div>
          Senate Holiday
          <div
            className={`${styles.legendBlock} ${styles.payPeriodCalPartialHoliday}`}
          >
            {"\u00A0"}
          </div>
...
const fetchPayPeriodApi = async (periodType, selectedYear) => {
  return fetchApiJson(`/periods/${periodType}?year=${selectedYear}`, {
    method: "GET",
  });
};
