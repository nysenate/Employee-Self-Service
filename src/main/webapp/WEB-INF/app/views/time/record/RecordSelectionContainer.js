...
export default function RecordSelectionContainer({
  state,
  getRecordRangeDisplay,
}) {
  const getDisplayText = (record) => {
    let overallUpdateDate = new Date(
      state.records[state.iSelectedRecord].overallUpdateDate,
    );
    let originalDate = new Date(
      state.records[state.iSelectedRecord].originalDate,
    );
    if (record) {
  ...
  return (
    <div
      className={`${styles.recordSelectionContainer} ${styles.contentContainer} ${styles.contentControls}`}
    >
          ...
          <tbody>
            {state.records.map((record, index) => (
              <tr key={index} onClick={() => (state.iSelectedRecord = index)}>
                ...
                <td>
                  {formatDateStandard(record.payPeriod.startDate)} -{" "}
                  {formatDateStandard(record.payPeriod.endDate)}
                </td>
                <td>{record.supervisor.fullName}</td>
                <td className={record.isDue ? styles.darkRed : ""}>
                  {record.dueFromNowStr}
                </td>
                {/*In ActiveAttendanceRecords below*/}
                <td>
                  <span
                    dangerouslySetInnerHTML={{
                      __html: timeRecordStatus(record.recordStatus, true),
                    }}
                  ></span>
                </td>
                <td> {getDisplayText(record)}</td>
              </tr>
            ))}
          </tbody>
        </table>
              ...
              >
                {state.records.map((record, index) => (
                  <option key={index} value={index}>
                    {getRecordRangeDisplay(record)}
                  </option>
                ))}
              </select>
            </label>
            <span>
              <span className={styles.bold}>Supervisor: </span>
              {state.records[state.iSelectedRecord].supervisor.fullName}
            </span>
            <span>
              <span className={styles.bold} style={{ marginLeft: "10px" }}>
                Status:{" "}
              </span>
              {state.records[state.iSelectedRecord].recordStatus}
            </span>
            <span className={styles.bold} style={{ marginLeft: "10px" }}>
              Last Updated:{" "}
            </span>
