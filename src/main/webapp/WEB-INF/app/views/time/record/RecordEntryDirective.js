...
export default function RecordEntryDirective() {
  ...
  return (
    <>
          ...
          {canCreateNextRecord() ? (
            <div className={styles.nextRecordForm}>
              <p className={styles.contentInfo}>
                All time records have been submitted up to the current pay
                period.
                <br />
                You may enter time for the next pay period by pressing the
                button below.
              </p>
              <input
                type="button"
      ...
      {/*SUBHERO*/}
      {!recordsLoading && state.records.length > 0 && (
        <RecordSelectionContainer
          state={state}
          getRecordRangeDisplay={getRecordRangeDisplay}
        />
      )}
      ...
      {errorTypes.record.prevUnsubmittedRecord && (
        ...
        <EssNotification level={"warn"} title={"Earlier Unsubmitted Records"}>
          <p>
            This record cannot be submitted until all previous annual salary
        ...
        (state.records[state.iSelectedRecord]?.recordStatus ===
          ...
          >
            <p style={{ marginTop: "20px" }}>
              {state.records[state.iSelectedRecord].initialRemarks}
            </p>
          </EssNotification>
        ))}
      ...
      {!recordsLoading && state.records[state.iSelectedRecord]?.timeEntries && (
        <div className={styles.contentContainer}>
          ...
          {state.annualEntries && state.tempEntries && (
            ...
            <EssNotification
              level={"warn"}
              title={"Record with multiple pay types"}
            >
              <p>
            ...
            <div className={styles.saveRecordContainer}>
              ...
              <div style={{ float: "right" }}>
                <input
                  ...
                  disabled={
                    !state.records[state.iSelectedRecord].dirty ||
                    !recordValid()
                  }
                  style={
                    !state.records[state.iSelectedRecord].dirty ||
                    !recordValid()
                      ? inlineDisabledStyles
                      : {}
                  }
                  value="Save Record"
                  onClick={() => saveRecord(false)}
                  tabIndex={
                    state.records[state.iSelectedRecord].dirty && recordValid()
                      ? 1
                      : -1
                  }
                />
