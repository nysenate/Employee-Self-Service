...
export default function Donation() {
  ...
  const submitDonation = async () => {
    try {
      await fetchApiJson(
        `/donation/submit?empId=${state.empId}&hoursToDonate=${state.hoursToDonate}`,
        {
          method: "POST",
          payload: {},
        },
  ...
  return (
    <div>
      ...
      <div style={{ textAlign: "center", fontSize: "120%" }}>
        ...
        {state.maxDonation >= 0.5 && (
          <div className={styles.contentContainer}>
            <p style={{ paddingTop: "10px" }}>
              You may donate up to {state.maxDonation} hours in half-hour
              increments.
            </p>
            <form>
      ...
      <div className={styles.contentControls} style={{ padding: "10px" }}>
        ...
        {state.donationData.length === 0 && (
          <div>You have no donations for this year yet.</div>
        )}
      </div>
...
function DonationContinuePopup({ isModalOpen, closeModal, onAction, state }) {
  ...
  return (
    <Popup isLocked={false} isOpen={isModalOpen} onClose={closeModal}>
      <div className={styles.confirmModal} style={{ fontSize: "120%" }}>
        <h3 className={styles.contentInfo}>Donation Confirmation</h3>
        <div className={styles.confirmationMessage}>
          You will donate {state.hoursToDonate} out of {state.accruedSickTime}{" "}
          accrued sick hours.
          <br />
          <hr />
