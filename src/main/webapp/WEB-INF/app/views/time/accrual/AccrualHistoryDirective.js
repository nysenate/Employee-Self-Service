...
const AccrualHistoryDirective = ({
  ...
  const getAccSummaries = async () => {
    ...
    try {
      const response = await fetchAccrualSummaries({
        empId: empId,
      ...
      const sortedSummaries = response.result
        ...
        .sort(
          (a, b) =>
            new Date(b.payPeriod.endDate) - new Date(a.payPeriod.endDate),
        );
  ...
  const shouldDisplayRecord = (accrualRecord) => {
    ...
    displayRecord =
      displayRecord && (!accrualRecord.computed || accrualRecord.submitted);
          ...
          {selectedYear ? (
            <div>
              ...
              {isLoading() ? (
                ...
                <>
                  {!accSummaries[selectedYear]?.length > 0 && (
                    <p className={styles.contentInfo}>
                      No historical accrual records exist for this year. If it
                      is early in the year they may not have been created yet.
                    </p>
                  )}
                  ...
                  {accSummaries[selectedYear]?.length > 0 && (
                    <div>
                      <p className={styles.contentInfo}>
                        Summary of historical accrual records. Click a row to
                        view or print a detailed summary of accrual hours.
                      </p>
                      ...
                      >
                        <thead>
                          <tr>
                            ...
                            <th colSpan="4" className={styles.personalHours}>
                              Personal Hours
                            </th>
                            <th colSpan="5" className={styles.vacationHours}>
                              Vacation Hours
                            </th>
                            <th colSpan="5" className={styles.sickHours}>
                              Sick Hours
                            </th>
                          </tr>
                          <tr>
                        ...
                        <tbody>
                          {accSummaries[selectedYear].map((record, index) => (
                            ...
                            >
                              ...
                              <td>
                                {new Date(
                                  new Date(record.payPeriod.endDate).setDate(
                                    new Date(
                              ...
                              <td
                                className={`${styles.accrualHours} ${styles.personal}`}
                              >
                                {record.personalAccruedYtd}
                              </td>
                              <td
                                className={`${styles.accrualHours} ${styles.personal}`}
                              >
                                {record.biweekPersonalUsed}
                              </td>
                              <td
                                className={`${styles.accrualHours} ${styles.personal}`}
                              >
                              ...
                              <td
                                className={`${styles.accrualHours} ${styles.availableHours} ${styles.personal}`}
                              >
                                {record.personalAvailable}
                              </td>

                              <td
                                className={`${styles.accrualHours} ${styles.vacation}`}
                              >
                                {record.vacationRate}
                              </td>
                              <td
                                className={`${styles.accrualHours} ${styles.vacation}`}
                              >
                                {record.vacationAccruedYtd +
                                  record.vacationBanked}
                              </td>
                              <td
                                className={`${styles.accrualHours} ${styles.vacation}`}
                              >
                                {record.biweekVacationUsed}
                              </td>
                              <td
                                className={`${styles.accrualHours} ${styles.vacation}`}
                              >
                              ...
                              <td
                                className={`${styles.accrualHours} ${styles.availableHours} ${styles.vacation}`}
                              >
                                {record.vacationAvailable}
                              </td>

                              <td
                                className={`${styles.accrualHours} ${styles.sick}`}
                              >
                                {record.sickRate}
                              </td>
                              <td
                              ...
                              <td
                                className={`${styles.accrualHours} ${styles.availableHours} ${styles.sick}`}
                              >
            ...
            <div>
              <p>
                {isUser() ? (
                  ...
                  <span>
                    {empSupInfo.empFirstName} {empSupInfo.empLastName} has{" "}
                  </span>
                )}
                no accrual records.
              </p>
              {empSupInfo?.senator && (
                <p>
                  {empSupInfo.firstName} {empSupInfo.lastName} is a Senator and
                  does not currently accrue time.
                </p>
              )}
            </div>
          )}
