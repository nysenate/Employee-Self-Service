...
export default function FulfillmentIndex() {
    ...
    const fetchData = async () => {
      try {
        ...
        const rejectedReqs = await initRejectedReqs(
          formatDateForApi(new Date(getCurrentDate())),
        );

        const allReqs = [...mostReqs, ...rejectedReqs];

        // Remove duplicates
        const uniqueReqs = allReqs.filter(
          (req, index, self) =>
            index ===
            self.findIndex((r) => r.requisitionId === req.requisitionId),
        );
  ...
  if (error) {
    return (
      <div>
        Error loading requisitions. Please reload the page and try again.
      </div>
    );
  }
