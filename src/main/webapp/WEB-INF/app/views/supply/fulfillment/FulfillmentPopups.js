...
import {
  ...
  formatDateYY,
  processRequisitionPost,
  rejectRequisitionPost,
  saveRequisitionPost,
  undoRequisitionPost,
} from "../helpers";
import { useNavigate } from "react-router-dom";

export function FulfillmentEditing({
  requisition,
  isModalOpen,
  closeModal,
  refreshData,
  ...
  const [editableRequisition, setEditableRequisition] = useState({
    ...requisition,
  });
  const [dirty, setDirty] = useState(false);

  const [displayRejectInstructions, setDisplayRejectInstructions] =
    useState(false);
  const [warning, setWarning] = useState(false);

  useEffect(() => {
    setDirty(
      JSON.stringify(originalRequisition) !==
        JSON.stringify(editableRequisition),
    );
  }, [editableRequisition]);
...
const OrderContent = ({
  ...
  const handleIssuerChange = (event) => {
    const selectedEmployee = supplyEmployees.find(
      (emp) => emp.employeeId == event.target.value,
    );
  ...
  const handleAddItem = () => {
    ...
    if (
      editableRequisition.lineItems.find(
        (obj) => obj.item.id === items.selected.id,
      )
    ) {
      setWarning(true);
      return;
  ...
  const redirectToFullHistory = () => {
    navigate(
      `/supply/order-history/order/${originalRequisition.requisitionId}`,
      { state: { order: originalRequisition } },
    );
  };
    ...
    <div className={`${styles.grid} ${styles.contentInfo}`}>
      <div className={styles.col812}>
        ...
        <div className={styles.paddingX} style={{ display: "block" }}>
          ...
          {warning && (
            <p style={{ color: "#e64727" }}>
              Item already exists in this order. Please adjust the quantity if
              it's not correct.
            </p>
          )}
        </div>
      ...
      {/* Right Margin */}
      <div
        className={`${styles.col412} ${styles.requisitionModalRightMargin}`}
      ...
      >
        ...
        <select
          value={editableRequisition.deliveryMethod || ""}
        ...
        <div style={{ textAlign: "center", paddingBottom: "20px" }}>
          ...
          <select
            value={editableRequisition.issuer?.employeeId || ""}
...
const EditableOrderListing = ({
  ...
  const handleQuantityChange = (id, newQuantity) => {
    ...
    const updatedLineItems = editableRequisition.lineItems.map((obj) =>
      obj.item.id === id
        ? { ...obj, quantity: isNaN(numericQuantity) ? "" : numericQuantity }
        : obj,
    );
  ...
  const sortedLineItems = editableRequisition.lineItems
    ? alphabetizeLineItems(editableRequisition.lineItems, "commodityCode")
    : [];
      ...
      <table className={`${styles.essTable} ${styles.supplyListingTable}`}>
        ...
        <tbody>
          {sortedLineItems.map((item) => (
            <tr
              key={item.item.id}
              className={calculateItemHighlighting(
                item,
                data.locationStatistics,
                editableRequisition.destination.locId,
              )}
            >
...
const ActionButtons = ({
  ...
  return (
    <div style={{ paddingTop: "10px", textAlign: "center" }}>
      {/* Undo Button */}
      {hasPermission("SUPPLY_REQUISITION_APPROVE") &&
      ...
      {/* Cancel Button */}
      <Button
        style={{ width: "15%", backgroundColor: "grey" }}
      ...
      {/* Save Button */}
      <Button style={{ width: "15%" }} onClick={handleSave} disabled={!dirty}>
        Save
      </Button>
      ...
      {originalRequisition.status === "PENDING" && (
        <Button
          style={{ width: "15%", backgroundColor: "#4196a7" }}
      ...
      {hasPermission("SUPPLY_REQUISITION_APPROVE") &&
        originalRequisition.status === "COMPLETED" && (
          <Button
            ...
            onClick={handleProcess}
            disabled={
              originalRequisition.customer.employeeId ==
              JSON.parse(localStorage.getItem("ess.auth.empId"))
            }
            onMouseEnter={() => {
              // Show popover logic
            }}
...
export function FulfillmentImmutable({ requisition, isModalOpen, closeModal }) {
  ...
  const acceptShipment = () => {
    console.log(
      "Implement whatever acceptShipment(requisition) does from fulfillment-immutable-module.jsp",
    );
  };

  const handlePrintClick = () => {
    navigate(`/supply/order-history/order/${requisition.requisitionId}`, {
      state: {
        order: requisition,
        print: true,
      },
      ...
      <div className={`${styles.grid} ${styles.contentInfo}`}>
        ...
        >
          ...
          {(!requisition.specialInstructions ||
            requisition.specialInstructions.length === 0) && (
            <div>No instructions provided for this requisition.</div>
          )}
          ...
          {requisition.status !== "CANCELED" && (
            <>
              ...
              <div>
                {requisition.approvedDateTime
                  ? formatDateYY(requisition.approvedDateTime)
                  : ""}
              </div>
            </>
          )}
