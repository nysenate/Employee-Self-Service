...
const Header = ({ filters, handleFilterChange }) => {
  ...
  useEffect(() => {
    const fetchOptions = async () => {
      try {
        const locations = await fetchApiJson(`/locations`).then(
          (body) => body.result,
        );
        const issuers = await fetchApiJson(`/supply/employees/issuers`).then(
          (body) => body.result,
        );
        const commodities = await fetchApiJson(`/supply/items`).then(
          (body) => body.result,
        );
  ...
  return (
    <div className={`${styles.contentContainer} ${styles.contentControls}`}>
      <h4
        className={`${styles.contentInfo} ${styles.supplyText}`}
      ...
      <div className={styles.grid} style={{ textAlign: "center" }}>
        ...
        <div className={styles.col612} style={{ padding: "0 10px 10px 10px" }}>
          ...
          <input
            ...
            value={fromDate}
            onChange={(e) =>
              handleFilterChange("from", new Date(e.target.value))
            }
          />
        </div>
        <div className={styles.col612} style={{ padding: "0 10px 10px 10px" }}>
