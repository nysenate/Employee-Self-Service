...
function alertCtrl($scope, $timeout, $filter, appProps, alertInfoApi) {
    ...
    function saveAlertInfo() {
        ...
        return $scope.state.request.savingAlertInfo = !0, ensureMobileOptions(), alertInfoApi.save({}, $scope.state.alertInfo, onSuccess, $scope.handleErrorResponse).$promise.then(init).then(setSaved).finally(postRequest);
    }
    function setSaved() {
...
}), angular.module("essMyInfo").controller("AlertCtrl", [ "$scope", "$timeout", "$filter", "appProps", "AlertInfoApi", alertCtrl ]);
