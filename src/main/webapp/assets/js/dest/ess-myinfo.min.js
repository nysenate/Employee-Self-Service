...
function emergencyNotificationCtrl($scope, $timeout, $filter, appProps, eniApi) {
    function init() {
        return $scope.state = angular.extend(initialState), getENI();
    }
    function getENI() {
        function onSuccess(resp) {
            $scope.state.eni = resp.result, $scope.state.eni.homePhone = $filter("phoneNumber")($scope.state.eni.homePhone), 
        ...
        function postRequest() {
            $scope.state.request.loadingEni = !1;
        }
        var params = {
            empId: $scope.state.empId
        };
        return $scope.state.request.loadingEni = !0, eniApi.get(params, onSuccess, $scope.handleErrorResponse).$promise.finally(postRequest);
    }
    function saveENI() {
        function onSuccess() {}
        function postRequest() {
            $scope.state.request.savingEni = !1;
        }
        return $scope.state.request.savingEni = !0, eniApi.save({}, $scope.state.eni, onSuccess, $scope.handleErrorResponse).$promise.then(init).then(setSaved).finally(postRequest);
    }
    function setSaved() {
    ...
    $scope.phoneErrorMsg = "Please enter a valid phone number", $scope.emailErrorMsg = "Please enter a valid email address";
    var initialState = {
    ...
    init(), $scope.saveENI = saveENI, $scope.isLoading = function() {
        var loading = !1;
...
}), angular.module("essMyInfo").controller("EmergencyNotificationCtrl", [ "$scope", "$timeout", "$filter", "appProps", "EmergencyNotificationInfoApi", emergencyNotificationCtrl ]);
