...
essApp.controller("MyInfoMainCtrl", [ "$scope", "$q", "appProps", "badgeService", "AckDocApi", "AcknowledgementApi", function($scope, $q, appProps, badgeService, ackDocApi, ackApi) {
...
essMyInfo.filter("formatDeductionHeader", function() {
    ...
    function acknowledgementCtrl($scope, $routeParams, $q, $location, bowser, appProps, modals, documentApi, ackApi) {
        function init() {
            $scope.state = angular.copy(initialState), $scope.state.docId = $routeParams.ackDocId, 
            $q.all([ getDocument(), getAcknowledgements() ]).then(processAcknowledgement);
        }
        ...
        function processAcknowledgement() {
            var acknowledgements = $scope.state.acknowledgements, docId = $scope.state.document.id;
        ...
        function setDocEmbedHeight() {
            var document = $scope.state.document, width = 840, heightFactor = document.totalHeight / document.maxWidth;
            $scope.state.document.pageCount;
        ...
        $scope.isLoading = function() {
            ...
            postAcknowledgement().then(function() {
                return $scope.updateAckBadge(), modals.open("acknowledge-success");
            }).then(function() {
                $location.url($scope.ackDocPageUrl);
            });
        }, $scope.markDocRead = function() {
            $scope.state.docRead = !0;
        }, $scope.isEdge = function() {
            return bowser.msedge;
        }, init();
    }
    angular.module("essMyInfo").controller("AckDocViewCtrl", [ "$scope", "$routeParams", "$q", "$location", "bowser", "appProps", "modals", "AckDocApi", "AcknowledgementApi", acknowledgementCtrl ]);
}(), function() {
    function acknowledgementCtrl($scope, $q, appProps, modals, documentApi, ackApi) {
        function init() {
            $scope.state = angular.copy(initialState), $q.all([ getDocuments(), getAcknowledgements() ]).then(sortDocuments);
        }
        ...
        function sortDocuments() {
            ...
            for (documents.acknowledged = [], documents.unacknowledged = []; documents.unsorted.length > 0; ) {
                ...
                $scope.state.acknowledgements[doc.id] ? documents.acknowledged.push(doc) : documents.unacknowledged.push(doc);
            }
        }
        var initialState = {
            documents: {
        ...
        $scope.isLoading = function() {
            ...
            if (acknowledgement) return acknowledgement.timestamp;
    ...
    angular.module("essMyInfo").controller("AcknowledgementCtrl", [ "$scope", "$q", "appProps", "modals", "AckDocApi", "AcknowledgementApi", acknowledgementCtrl ]);
