...
essMyInfo.controller("EmpCheckHistoryCtrl", [ "$scope", "$filter", "appProps", "EmpCheckHistoryApi", "EmpActiveYearsApi", "modals", function($scope, $filter, appProps, EmpCheckHistoryApi, EmpActiveYearsApi, modals) {
    ...
    $scope.ytd = null, $scope.init = function() {
        ...
        EmpCheckHistoryApi.get(params, function(response) {
            ...
            $scope.checkHistory.searching = !1, $scope.handleErrorResponse(response);
        });
...
essMyInfo.filter("formatDeductionHeader", function() {
    ...
    angular.module("essMyInfo").controller("AckDocReportCtrl", [ "$scope", "$q", "appProps", "modals", "AckDocApi", "AcknowledgmentYearApi", ackDocReportCtrl ]);
}(), angular.module("essMyInfo").directive("ackDocSelect", [ "appProps", "AckDocApi", "AcknowledgmentYearApi", function(appProps, AckDocApi, AcknowledgmentYearApi) {
    function link($scope, $ele, $attrs) {
        function getAckDocsInAYear() {
            ...
            if ($scope.year) {
                ...
                var params = {
                    year: $scope.year
                }, requestAcquireAcks = !0;
                return AckDocApi.get(params, onSuccess, $scope.handleErrorResponse).$promise.finally(function() {
                    requestAcquireAcks = !1;
                });
            }
        }
        $scope.docYears = [], $scope.year = null, $scope.$watch("year", getAckDocsInAYear), 
        getAckDocYears($scope, !0), $scope.selectDocYear = function(docYear) {
            getAckDocsInAYear();
        };
    }
    function getAckDocYears($scope, requestGetYears) {
        ...
        return AcknowledgmentYearApi.get(onSuccess, $scope.handleErrorResponse).$promise.finally(function() {
            requestGetYears = !1;
        });
    }
    return {
...
function() {
    function empAckDocReportCtrl($scope, modals, EmpAckReportApi, AckApi) {
        ...
        function onSelectedEmpChange() {
            function onSuccess(resp) {
                resp.acks.forEach(function(ack) {
                    ...
                    $scope.state.yearDocMap[year] || ($scope.state.yearDocMap[year] = []), $scope.state.yearDocMap[year].push(ack);
                });
                var years = Object.keys($scope.state.yearDocMap).map(parseInt);
                years.sort().reverse(), $scope.state.years = years, $scope.state.selectedYear = years[0];
            }
            if (resetState(), $scope.selectedEmp) {
                var params = {
                    empId: $scope.selectedEmp.empId
                };
                return $scope.state.loadingAcks = !0, EmpAckReportApi.get(params, onSuccess, $scope.handleErrorResponse).$promise.finally(function() {
                    $scope.state.loadingAcks = !1;
                });
            }
        }
        function onSelectedYearChange() {
            if ($scope.state.selectedYear) {
                var ackStatuses = $scope.state.yearDocMap[$scope.state.selectedYear] || [], unacked = $scope.state.unacknowledged = [], acked = $scope.state.acknowledged = [];
                ackStatuses.forEach(function(status) {
                    null === status.ack ? unacked.push(status) : acked.push(status);
                });
            }
        }
        function postPersonnelAck(ackDoc) {
            function onSuccess() {
                onSelectedEmpChange(), showPersonnelAckSuccessModal(ackDoc);
            }
            var params = {
                empId: $scope.selectedEmp.empId,
                ackDocId: ackDoc.id
        ...
        function showPersonnelAckSuccessModal(ackDoc) {
            $scope.state.selectedDoc = ackDoc, modals.open("personnel-acknowledge-success");
        }
        var initialState = {
            loadingAcks: !1,
            yearDocMap: {},
        ...
        resetState(), $scope.$watch("selectedEmp", onSelectedEmpChange), $scope.$watch("state.selectedYear", onSelectedYearChange), 
        $scope.initiatePersonnelAck = function(ackStatus) {
            $scope.state.selectedDoc = ackStatus.ackDoc, modals.open("personnel-acknowledge-prompt").then(function() {
                postPersonnelAck(ackStatus.ackDoc);
            });
        };
    }
    angular.module("essMyInfo").controller("EmpAckDocReportCtrl", [ "$scope", "modals", "EmpAckReportApi", "AcknowledgmentApi", empAckDocReportCtrl ]);
}();
...
essMyInfo.controller("EmpTransactionHistoryCtrl", [ "$scope", "$http", "appProps", "$q", "EmpTransactionTimelineApi", "modals", function($scope, $http, appProps, $q, EmpTransactionTimelineApi, modals) {
    $scope.state = {
        ...
        return EmpTransactionTimelineApi.get({
            ...
            $scope.state.timeline = !1, $scope.handleErrorResponse(resp), deferred.reject("Failed to retrieve timeline.");
        }), deferred;
    }, $scope.init = function() {
        $scope.getTimeline();
    }();
