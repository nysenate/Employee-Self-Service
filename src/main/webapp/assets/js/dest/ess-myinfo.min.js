        ...
        function onSelectedEmpChange() {
            function onSuccess(resp) {
                angular.forEach(resp.acks, function(ackStatus) {
                    var status = ackStatus.ackDoc.title + "-" + new Date(ackStatus.ackDoc.effectiveDateTime).getFullYear() + " ";
                    null == ackStatus.ack ? status += " WAS NOT ACKNOWLEDGED" : status = status + " Acked on: " + ackStatus.ack.timestamp, 
                    $scope.displayAckStatuses.push(status);
                }), $scope.ackStatusesReady = !0;
            }
            if ($scope.displayAckStatuses = [], $scope.ackStatusesReady = !1, $scope.selectedEmp) {
                var params = {
                    empId: $scope.selectedEmp.empId
                }, requestAcquireAcks = !0;
                return EmpAckReportApi.get(params, onSuccess, $scope.handleErrorResponse).$promise.finally(function() {
                    requestAcquireAcks = !1;
                });
            }
        }
        $scope.ackStatusesReady = !1, $scope.displayAckStatuses = [], $scope.$watch("selectedEmp", onSelectedEmpChange);
    }
    angular.module("essMyInfo").controller("EmpAckDocReportCtrl", [ "$scope", "$q", "appProps", "modals", "EmpAckReportApi", empAckDocReportCtrl ]);
}();
