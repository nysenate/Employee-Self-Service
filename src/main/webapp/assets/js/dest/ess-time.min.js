...
function accrualProjectionCtrl($scope, $timeout, appProps, AccrualHistoryApi, EmpInfoApi, modals, accrualUtils) {
    ...
    $scope.state = {
        ...
        var summLen = ($scope.state.projections.length, $scope.state.accSummaries.length), baseRec = null;
        baseRec = summLen > 0 ? $scope.state.accSummaries[0] : $scope.state.projections[0];
        for (var multiYear = !1, per = baseRec.personalUsed, vac = baseRec.vacationUsed, sick = baseRec.empSickUsed + baseRec.famSickUsed, i = 0; i < $scope.state.projections.length; i++) {
            var rec = $scope.state.projections[i], lastRec = $scope.state.projections[i - 1];
            multiYear && (rec.vacationBanked = lastRec.vacationBanked, rec.sickBanked = lastRec.sickBanked), 
            i > 0 && accrualUtils.isFirstRecordOfYear(rec) && (multiYear = !0, rec.vacationBanked = Math.min(lastRec.vacationAvailable, maxVacationBanked), 
            rec.sickBanked = Math.min(lastRec.sickAvailable, maxSickBanked), per = vac = sick = 0), 
...
function accrualUtils() {
    function computeDeltas(accruals) {
        for (var i = 0; i < accruals.length; i++) {
            ...
            if (0 == i || isFirstRecordOfYear(currSummary)) currSummary.vacationUsedDelta = currSummary.vacationUsed, 
            currSummary.personalUsedDelta = currSummary.personalUsed, currSummary.sickUsedDelta = currSummary.empSickUsed + currSummary.famSickUsed; else {
        ...
        link: function($scope, $elem, $attrs) {
            ...
            function loadAccruals() {
                if ($scope.showAccruals) {
                    var record = $scope.record, empId = record.employeeId, periodStartMoment = moment(record.payPeriod.startDate), params = {
                        empId: empId,
                        beforeDate: periodStartMoment.format("YYYY-MM-DD")
                    };
            ...
            $scope.close = modals.reject, $scope.tempEntries = $scope.annualEntries = !1, $scope.showExitBtn = "false" !== $attrs.exitBtn, 
            $scope.showAccruals = showAccrualsSelected, $scope.loadingAccruals = !1, $scope.$watch("record", function(record) {
