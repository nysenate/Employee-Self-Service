...
function recordEntryCtrl($scope, $rootScope, $filter, $q, $timeout, appProps, activeRecordsApi, recordSaveApi, accrualPeriodApi, allowanceApi, miscLeaveGrantApi, holidayApi, recordCreationApi, activeRow, recordUtils, locationService, modals, promiseUtils) {
    ...
    $scope.state = null, $scope.initializeState = function() {
        ...
        var record = $scope.state.records[$scope.state.iSelectedRecord];
        if (fullValidationCheck(), $scope.selRecordHasEntryErrors() || submit && $scope.selRecordHasRecordErrors()) return void $scope.$broadcast("validateRecordEntries");
        ...
        return !(null == $scope.getSelectedRecord() || $scope.selRecordHasEntryErrors());
        ...
        $scope.getSelectedRecord().focused = !0;
        ...
        for (var iRecord in $scope.state.allRecords) if ($scope.state.allRecords.hasOwnProperty(iRecord)) {
            ...
            if (now.isBefore(record.beginDate)) return !1;
        }
        return !0;
        ...
        raSa: {
            workHours: function(entry) {
                ...
                var isValid = !0;
                return void 0 === hrs && ($scope.errorTypes.raSa.workHoursInvalidRange = !0, isValid = !1), 
            ...
            holidayHours: function(entry) {
                ...
                return 0 === hrs || null === hrs || (void 0 === hrs && ($scope.errorTypes.raSa.holidayHoursInvalidRange = !0, 
            ...
            vacationHours: function(entry) {
                ...
                isValid = !1), void 0 === hrs && ($scope.errorTypes.raSa.vacationHoursInvalidRange = !0, 
                isValid = !1), isValid &= checkRaSaHourIncrements(hrs));
            },
            personalHours: function(entry) {
                ...
                isValid = !1), void 0 === hrs && ($scope.errorTypes.raSa.personalHoursInvalidRange = !0, 
                isValid = !1), isValid &= checkRaSaHourIncrements(hrs));
            },
            sickEmpHours: function(entry) {
                var hrs = entry.sickEmpHours, isValid = !0;
                return 0 === hrs || null === hrs || (isValid &= isEnoughSickTime(), void 0 === hrs && ($scope.errorTypes.raSa.sickEmpHoursInvalidRange = !0, 
                isValid = !1), isValid &= checkRaSaHourIncrements(hrs));
            },
            sickFamHours: function(entry) {
                var hrs = entry.sickFamHours, isValid = !0;
                return 0 === hrs || null === hrs || (isValid &= isEnoughSickTime(), void 0 === hrs && ($scope.errorTypes.raSa.sickFamHoursInvalidRange = !0, 
                isValid = !1), isValid &= checkRaSaHourIncrements(hrs));
            },
            miscHours: function(entry) {
                var hrs = entry.miscHours, isValid = !0;
                return 0 === hrs || null === hrs || (void 0 === hrs && ($scope.errorTypes.raSa.miscHoursInvalidRange = !0, 
                isValid = !1), isValid &= checkRaSaHourIncrements(hrs));
            },
            miscType: function(entry) {
                var miscTypePresent = null !== entry.miscType, miscHoursPresent = entry.miscHours > 0;
                return entry.index === activeRow.getActiveRow() || miscTypePresent || !miscHoursPresent ? !(miscTypePresent && !miscHoursPresent && ($scope.errorTypes.raSa.noMiscHoursGiven = !0, 
        ...
        te: {
            workHours: function(entry) {
                ...
                if (0 === hrs || null === hrs) return !0;
                var isValid = !0;
                return void 0 === hrs && ($scope.errorTypes.te.workHoursInvalidRange = !0, isValid = !1), 
...
function recordManageCtrl($scope, $q, $filter, appProps, recordUtils, modals, badgeService, supRecordsApi, timeRecordsApi, reminderApi) {
    ...
    function initializeRecords(recordMap) {
        ...
        angular.forEach(recordMap, function(records, empId) {
            angular.forEach(records, function(record) {
                ...
                $scope.state.supIds.indexOf(currSupId) == -1 && ($scope.state.supIds.push(currSupId), 
    ...
    $scope.init = function() {
        ...
        modals.open("record-review", params, !allowApproval).then(submitReviewedRecords).then($scope.selectNone);
...
function recordReviewModal(appProps, modals, locationService) {
    function link($scope, $elem, $attrs) {
        ...
        $elem.addClass("custom-fit"), $scope.submitChanges = function() {
            ...
            if ($scope.submissionEmpty()) return void modals.reject();
            modals.open("record-review-close").then(modals.reject);
...
function empRecordHistoryCtrl($scope, $q, $timeout, appProps, ActiveYearsTimeRecordsApi, TimeRecordsApi, AttendanceRecordApi, SupervisorEmployeesApi, modals, recordUtils) {
    function handleEmpGroupResponse(resp) {
        $scope.state.extSupEmpGroup = resp.result, setNameMap(), setSupEmpGroups(), $scope.state.iSelEmpGroup = 0;
    }
    function getTimeRecordYears() {
        var iSelEmp = $scope.state.iSelEmp;
        if (!(iSelEmp < 0)) {
            var emp = $scope.state.allEmps[iSelEmp];
            return $scope.state.selectedRecYear = -1, $scope.state.request.tRecYears = !0, ActiveYearsTimeRecordsApi.get({
                empId: emp.empId
            }, function(resp) {
                if (resp.success) {
                    ...
                    }).reverse(), $scope.state.selectedRecYear = $scope.state.recordYears[0];
                }
            }, function(resp) {
                modals.open("500", {
                    details: resp
    ...
    function getRecords() {
        var iSelEmp = $scope.state.iSelEmp;
        if (!(iSelEmp < 0)) {
            var emp = $scope.state.allEmps[iSelEmp], year = $scope.state.selectedRecYear;
            if (!(year < 0)) {
                ...
                if (startMoment.isAfter(emp.supEndMoment) || endMoment.isBefore(emp.supStartMoment)) return void ($scope.state.records = []);
                $scope.state.supStartDate = moment.max(startMoment, emp.supStartMoment), $scope.state.supEndDate = moment.min(endMoment, emp.supEndMoment), 
                $scope.state.request.records = !0, $q.all([ $scope.getTimesheetRecords(), $scope.getAttendRecords() ]).then(function() {
    ...
    function setActiveSupEmpGroup() {
        if (!($scope.state.iSelEmpGroup < 0)) {
            var selEmpGroup = $scope.state.supEmpGroups[$scope.state.iSelEmpGroup], isUser = selEmpGroup.supId === $scope.state.supId;
            $scope.state.primaryEmps = selEmpGroup.primaryEmployees.sort(function(a, b) {
                return a.empLastName.localeCompare(b.empLastName);
            });
            var primaryEmpLookup = {}, empList = [];
            angular.forEach($scope.state.primaryEmps, function(emp) {
                ...
                emp.group = "Additional Employees", setAdditionalEmpData(emp), empList.push(emp);
                ...
                angular.forEach(supGroup, function(emp) {
                    ...
                    setAdditionalEmpData(emp), empList.push(emp);
                });
            })), $scope.state.iSelEmp = -1, $timeout(function() {
                $scope.state.iSelEmp = 0;
            }), $scope.state.allEmps = empList;
        }
    }
    function setAdditionalEmpData(emp) {
        emp.supStartMoment = moment(emp.supStartDate || "1970-01-01"), emp.supEndMoment = moment(emp.supEndDate || "2999-12-31"), 
        emp.dropDownLabel = emp.empLastName + " " + emp.empFirstName[0] + ". (" + emp.supStartMoment.format("MMM YYYY") + " - " + (emp.supEndMoment.isBefore() ? emp.supEndMoment.format("MMM YYYY") : "Present") + ")";
    }
    function setNameMap() {
        var extSupEmpGroup = $scope.state.extSupEmpGroup, primaryEmpInfos = extSupEmpGroup.primaryEmployees, empOverrideInfos = extSupEmpGroup.empOverrideEmployees, supOverrideInfos = Object.keys(extSupEmpGroup.supOverrideEmployees).map(function(k) {
            return extSupEmpGroup.supOverrideEmployees[k];
        }), allEmpInfos = primaryEmpInfos.concat(empOverrideInfos).concat(supOverrideInfos);
    ...
    function setSupEmpGroups() {
        var extSupEmpGroup = $scope.state.extSupEmpGroup, empSupEmpGroups = [];
        angular.forEach(extSupEmpGroup.employeeSupEmpGroups, function(supEmpGroups) {
            angular.forEach(supEmpGroups, function(empGroup) {
                ...
                empGroup.empFirstName = $scope.state.nameMap[empGroup.supId].firstName, empGroup.empLastName = $scope.state.nameMap[empGroup.supId].lastName, 
                setAdditionalEmpData(empGroup), empGroup.group = "Employees", empSupEmpGroups.push(empGroup);
            });
        }), empSupEmpGroups.sort(function(a, b) {
            var aLabel = a.dropDownLabel, bLabel = b.dropDownLabel;
    ...
    function combineRecords() {
        ...
        angular.forEach($scope.state.attendRecords, function(attendRecord) {
            if (moment(attendRecord.endDate).isAfter(attendEnd) && (attendEnd = attendRecord.endDate), 
            0 === attendRecord.timesheetIds.length) return $scope.state.paperTimesheetsDisplayed = !0, 
            void $scope.state.records.push(attendRecord);
    ...
    $scope.state = {
        ...
        primaryEmps: [],
        primarySups: []
    }, $scope.$watch("state.iSelEmpGroup", setActiveSupEmpGroup), $scope.$watch("state.iSelEmp", getTimeRecordYears), 
    $scope.$watch("state.selectedRecYear", getRecords), $scope.getEmployeeGroups = function(supId, fromDate, toDate) {
        var fromDateMoment = fromDate ? moment(fromDate) : moment().subtract(2, "years"), toDateMoment = toDate ? moment(toDate) : moment(), params = {
            supId: supId,
            fromDate: fromDateMoment.format("YYYY-MM-DD"),
            toDate: toDateMoment.format("YYYY-MM-DD"),
            extended: !0
        };
        $scope.state.request.empGroups = !0, SupervisorEmployeesApi.get(params, handleEmpGroupResponse, $scope.handleErrorResponse).$promise.finally(function() {
            $scope.state.request.empGroups = !1;
        });
    }, $scope.getTimesheetRecords = function() {
        var emp = $scope.state.allEmps[$scope.state.iSelEmp], params = {
            empId: emp.empId,
...
function recordValidatorDirective($timeout, $rootScope, debounce, activeRow) {
    return {
        ...
        link: function($scope, $elem, $attrs) {
            ...
            var debouncedValidate = debounce(validate, 350);
...
function accrualHistoryCtrl($scope, $timeout, appProps, AccrualHistoryApi, EmpActiveYearsApi, EmpInfoApi, timeRecordsApi, modals, accrualUtils) {
    function getEmpInfo() {
    ...
    function reflowTable(count) {
        count > 20 || !$scope.state.accSummaries[$scope.state.selectedYear] || (count = isNaN(count) ? 0 : count, 
    ...
    $scope.state = {
        ...
        AccrualHistoryApi.get({
            ...
            resp.success && ($scope.state.error = null, $scope.state.accSummaries[year] = resp.result.filter(function(acc) {
        ...
        var empId = appProps.user.employeeId, fromDate = (moment(), moment([ year, 0, 1 ])), toDate = moment([ year + 1, 0, 1 ]).subtract(1, "days"), params = {
            empId: $scope.state.empId,
    ...
    $scope.viewDetails = function(accrualRecord) {
        ...
        $scope.getEmpActiveYears(function() {
            $scope.getAccSummaries($scope.state.selectedYear), $scope.getTimeRecords($scope.state.selectedYear);
        });
    }();
}
...
function accrualProjectionCtrl($scope, $timeout, appProps, AccrualHistoryApi, EmpInfoApi, modals, accrualUtils) {
    ...
    $scope.state = {
        ...
        $scope.state.searching = !0;
        var fromDate = moment([ year, 0, 1 ]).subtract(6, "months"), toDate = moment([ year + 1, 0, 1 ]).subtract(1, "days");
        AccrualHistoryApi.get({
            empId: $scope.state.empId,
            fromDate: fromDate.format("YYYY-MM-DD"),
            toDate: toDate.format("YYYY-MM-DD")
        }, function(resp) {
            resp.success && ($scope.state.error = null, $scope.state.accSummaries = resp.result.filter(function(acc) {
                return !acc.computed || acc.submitted;
            }).reverse(), year >= $scope.state.today.year() - 1 && ($scope.state.projections = resp.result.filter(isValidProjection).map(initializeProjection))), 
        ...
        var accSummaries = $scope.state.accSummaries, projections = $scope.state.projections, baseRec = accSummaries.length > 0 ? accSummaries[0] : null, multiYear = !1, per = 0, vac = 0, sickEmp = 0, sickFam = 0;
        baseRec && (per = baseRec.personalUsed, vac = baseRec.vacationUsed, sickEmp = baseRec.sickEmpUsed, 
        ...
        for (var i = 0; i < projections.length; i++) {
            ...
            lastRec && accrualUtils.isFirstRecordOfYear(rec) && (multiYear = !0, rec.vacationBanked = Math.min(lastRec.vacationAvailable, 210), 
            rec.sickBanked = Math.min(lastRec.sickAvailable, 1400), per = vac = sickEmp = sickFam = 0), 
            per += rec.biweekPersonalUsed || 0, vac += rec.biweekVacationUsed || 0, sickEmp += rec.biweekSickEmpUsed || 0, 
            sickFam += rec.biweekSickFamUsed || 0, rec.personalUsed = per, rec.vacationUsed = vac, 
            rec.sickEmpUsed = sickEmp, rec.sickFamUsed = sickFam, rec.holidayUsed = rec.holidayUsed || 0, 
    ...
    $scope.$watchCollection("state.projections", reflowTable), $scope.getAccrualReportURL = accrualUtils.getAccrualReportURL, 
    $scope.init();
}
        ...
        link: function($scope, $elem, $attrs) {
            function detectPayTypes() {
                angular.forEach($scope.record.timeEntries, function(entry) {
                    $scope.tempEntries = $scope.annualEntries = !1, $scope.tempEntries = $scope.tempEntries || "TE" === entry.payType, 
            ...
            $scope.loadingAccruals = !1, $scope.$watch("record", function(record) {
                record && (detectPayTypes(), loadAccruals());
            });
        }
    };
...
var essApp = angular.module("essTime").controller("RecordEntryController", [ "$scope", "$rootScope", "$filter", "$q", "$timeout", "appProps", "ActiveTimeRecordsApi", "TimeRecordApi", "AccrualPeriodApi", "AllowanceApi", "MiscLeaveGrantApi", "HolidayApi", "TimeRecordCreationApi", "activeTimeEntryRow", "RecordUtils", "LocationService", "modals", "promiseUtils", recordEntryCtrl ]), essTime = angular.module("essTime");
...
essApp.controller("RecordManageCtrl", [ "$scope", "$q", "$filter", "appProps", "RecordUtils", "modals", "badgeService", "SupervisorTimeRecordsApi", "TimeRecordApi", "TimeRecordReminderApi", recordManageCtrl ]);
...
essTime.controller("EmpRecordHistoryCtrl", [ "$scope", "$q", "$timeout", "appProps", "ActiveYearsTimeRecordsApi", "TimeRecordApi", "AttendanceRecordApi", "SupervisorEmployeesApi", "modals", "RecordUtils", empRecordHistoryCtrl ]), 
...
angular.module("essTime").controller("NewRequestCtrl", [ "$scope", "$http", function($scope, $http) {
...
essTime.controller("AccrualHistoryCtrl", [ "$scope", "$timeout", "appProps", "AccrualHistoryApi", "EmpActiveYearsApi", "EmpInfoApi", "TimeRecordApi", "modals", "AccrualUtils", accrualHistoryCtrl ]);
...
angular.module("essTime").service("AccrualUtils", accrualUtils), angular.module("essTime").directive("accrualBar", [ "appProps", accrualBarDirective ]), 
