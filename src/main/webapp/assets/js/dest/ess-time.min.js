...
function recordManageCtrl($scope, $q, $filter, appProps, recordUtils, modals, badgeService, supEmpGroupService, supRecordsApi, timeRecordsApi, reminderApi) {
    ...
    function getPrimarySupEntries() {
        var primarySupEntries = [], supId = appProps.user.employeeId, extSupEmpGroup = supEmpGroupService.getExtSupEmpGroup(), supName = supEmpGroupService.getName(supId), hasSupOverrides = extSupEmpGroup.supOverrideEmployees.size > 0, hasEmpOverrides = extSupEmpGroup.empOverrideEmployees.length > 0;
        if (hasSupOverrides || hasEmpOverrides) {
            var fullEmpGroupBaseLabel = supName.fullName + " + Overrides", fullEmpGroupEntry = {
                querySupId: supId,
                supId: supId,
                name: supName,
        ...
        return primarySupEntries.push(supEntry), primarySupEntries;
    }
    ...
    function getExtendedSupEntries() {
        var supEmpGroups = supEmpGroupService.getSupEmpGroupList(), supName = supEmpGroupService.getName(appProps.user.employeeId), extSupGroup = "Supervisors Under " + supName.fullName, extSupEntries = supEmpGroups.slice(1).filter(function(empGroup) {
    ...
    function initializeRecords(supId, recordMap) {
        ...
        affectedSupervisors.forEach(function(supEntry) {
            ...
            angular.forEach(records, function(record) {
                (isUser || record.supervisorId === supId) && (recordUtils.calculateDailyTotals(record), 
...
function employeeSelectDirective(appProps, supEmpGroupService) {
    return {
        ...
        link: function($scope, $elem, $attrs) {
            ...
            $scope.activeOnly = "true" === ($attrs.activeOnly || "").toLowerCase(), $scope.selectSubject = $attrs.selectSubject, 
...
essApp.controller("RecordManageCtrl", [ "$scope", "$q", "$filter", "appProps", "RecordUtils", "modals", "badgeService", "supEmpGroupService", "SupervisorTimeRecordsApi", "TimeRecordApi", "TimeRecordReminderApi", recordManageCtrl ]);
