...
function allowanceUtils($filter) {
    function computeRemaining(allowance, record) {
        var highestRate = 0;
        angular.forEach(allowance.salaryRecs, function(salaryRec) {
            "TE" !== salaryRec.payType || moment(salaryRec.effectDate).isAfter(record.endDate) || moment(record.beginDate).isAfter(salaryRec.endDate) || salaryRec.salaryRate > highestRate && (highestRate = salaryRec.salaryRate);
