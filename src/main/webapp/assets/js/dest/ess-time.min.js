function empRecordHistoryCtrl($scope, $q, $timeout, appProps, modals, recordUtils, supEmpGroupService, ActiveYearsTimeRecordsApi, TimeRecordsApi, AttendanceRecordApi, SupervisorEmployeesApi) {
    ...
    function getRecords() {
        ...
        if (emp.empId) {
            ...
            if (!(year < 0)) {
                ...
                var isUserSup = emp.supId === $scope.state.supId, supStartDate = isUserSup ? emp.supStartDate : emp.effectiveStartDate, supEndDate = isUserSup ? emp.supEndDate : emp.effectiveEndDate, startMoment = moment([ year, 0, 1 ]), endMoment = $scope.state.todayMoment.year() == year ? moment() : moment([ year, 11, 31 ]);
                if (startMoment.isAfter(supEndDate || void 0) || endMoment.isBefore(supStartDate || 0)) return void ($scope.state.records = []);
                $scope.state.supStartDate = moment.max(startMoment, moment(supStartDate || 0)), 
                $scope.state.supEndDate = moment.min(endMoment, moment(supEndDate || void 0)), $scope.state.request.records = !0, 
                $q.all([ getTimesheetRecords(), getAttendRecords() ]).then(function() {
    ...
    function getTimesheetRecords() {
        var emp = $scope.state.selectedEmp, params = {
            empId: emp.empId,
    ...
    $scope.state = {
        ...
        timesheetRecords: [],
        attendRecords: []
    }, $scope.$watchCollection("state.selectedEmp", getTimeRecordYears), $scope.$watch("state.selectedRecYear", getRecords), 
    function() {
        $scope.state.request.empGroups = !0, supEmpGroupService.init().finally(function() {
    ...
    return {
        ...
        link: function($scope, $elem, $attrs) {
            function setSelectedEmployeeGroup() {
                $scope.iSelEmpGroup < 0 || ($scope.allEmps = supEmpGroupService.getEmpInfos($scope.iSelEmpGroup), 
                setEmpLabels(), $scope.selectedSup = $scope.supEmpGroups[$scope.iSelEmpGroup], 0 === $scope.iSelEmp ? setSelectedEmployee() : $scope.iSelEmp = 0);
            }
            ...
            function setSupGroupLabels() {
                ...
                angular.forEach($scope.supEmpGroups, function(empGroup) {
                    empGroup.supId === appProps.user.employeeId ? empGroup.dropDownLabel = supName : (empGroup.group = "Employees Under " + supName, 
            ...
            function setEmpLabels() {
                angular.forEach($scope.allEmps, function(emp) {
                    if (emp.empOverride) emp.group = "Employee Overrides"; else if (emp.supOverride) {
                        ...
                        emp.group = supName && supName.lastName ? supName.lastName + "'s Employees" : "Sup Override Employees";
...
function supEmpGroupService($filter, appProps, modals, supEmployeeApi) {
    ...
    function loadSupEmpGroup() {
        function onSuccess(response) {
            extendedSupEmpGroup = response.result, setNameMap(), setSupEmpGroups();
        }
        ...
        var fromDateMoment = moment().subtract(2, "years"), toDateMoment = moment(), params = {
            supId: appProps.user.employeeId,
    ...
    function setSupEmpGroups() {
        ...
        }), empSupEmpGroups = $filter("orderBy")(empSupEmpGroups, [ "empLastName", "empfirstName", "supId", "effectiveEndDate" ]), 
    ...
    var extendedSupEmpGroup = null, supEmpGroupList = [], nameMap = {}, empGroupPromise = null;
    return init(), {
        init: init,
        ...
        link: function($scope) {
            ...
            function getEmpActiveYears() {
                if ($scope.empId) {
                    ...
                    $scope.request.empActiveYears = !0, EmpActiveYearsApi.get(params, function(resp) {
                        $scope.activeYears = resp.activeYears.reverse(), $scope.selectedYear = resp.activeYears[0];
                    }, function(resp) {
                        modals.open("500", {
                            details: resp
                        });
            ...
            function getAccSummaries() {
                ...
                if (year && !$scope.accSummaries[year]) {
                    var fromDate = moment([ year, 0, 1 ]), toDate = moment([ year + 1, 0, 1 ]).subtract(1, "days"), params = {
                        empId: $scope.empId,
                        fromDate: fromDate.format("YYYY-MM-DD"),
                        toDate: toDate.format("YYYY-MM-DD")
                    };
            ...
            $scope.empId || ($scope.empId = appProps.user.employeeId), $scope.accSummaries = {}, 
            $scope.activeYears = [], $scope.timeRecords = [], $scope.selectedYear = null, $scope.empInfo = {}, 
            $scope.isTe = !1, $scope.error = null, $scope.request = {
                ...
                scrollingTop: 47,
                useAbsolutePositioning: !1
            }, $scope.$watch("empId", getEmpInfo), $scope.$watch("empId", getEmpActiveYears), 
            $scope.$watch("selectedYear", getAccSummaries), $scope.isLoading = function() {
...
essTime.controller("EmpRecordHistoryCtrl", [ "$scope", "$q", "$timeout", "appProps", "modals", "RecordUtils", "supEmpGroupService", "ActiveYearsTimeRecordsApi", "TimeRecordApi", "AttendanceRecordApi", "SupervisorEmployeesApi", empRecordHistoryCtrl ]);

var essApp = angular.module("essTime").controller("RecordEntryController", [ "$scope", "$rootScope", "$filter", "$q", "$timeout", "appProps", "ActiveTimeRecordsApi", "TimeRecordApi", "AccrualPeriodApi", "AllowanceApi", "MiscLeaveGrantApi", "HolidayApi", "TimeRecordCreationApi", "activeTimeEntryRow", "RecordUtils", "LocationService", "modals", "promiseUtils", recordEntryCtrl ]);
...
} ]), angular.module("essTime").directive("employeeSelect", [ "appProps", "supEmpGroupService", employeeSelectDirective ]), 
...
angular.module("essTime").directive("accrualHistory", [ "$timeout", "appProps", "modals", "AccrualHistoryApi", "EmpInfoApi", "EmpActiveYearsApi", accrualHistoryDirective ]);
