...
function empRecordHistoryCtrl($scope, $q, appProps, ActiveYearsTimeRecordsApi, TimeRecordsApi, AttendanceRecordApi, SupervisorEmployeesApi, modals, recordUtils) {
    function setAdditionalEmpData(emp) {
        ...
        emp.dropDownLabel = emp.empLastName + " " + emp.empFirstName[0] + ". (" + emp.supStartMoment.format("MMM YYYY") + " - " + emp.supEndMoment.format("MMM YYYY") + ")";
    }
...
function accrualProjectionCtrl($scope, $timeout, appProps, AccrualHistoryApi, EmpInfoApi, modals, accrualUtils) {
    ...
    $scope.state = {
        ...
        var accSummaries = $scope.state.accSummaries, projections = $scope.state.projections, baseRec = accSummaries.length > 0 ? accSummaries[0] : null, multiYear = !1, per = 0, vac = 0, sick = 0;
        baseRec && (per = baseRec.personalUsed, vac = baseRec.vacationUsed, sick = baseRec.empSickUsed + baseRec.famSickUsed);
