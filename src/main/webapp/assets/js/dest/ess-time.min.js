...
function recordEntryCtrl($scope, $rootScope, $filter, $q, $timeout, appProps, activeRecordsApi, recordSaveApi, accrualPeriodApi, allowanceApi, miscLeaveGrantApi, holidayApi, activeRow, recordUtils, locationService, modals, promiseUtils) {
    ...
    function saveRecord(submit) {
        ...
        return $scope.state.request.save = !0, modals.open("save-indicator", {
            submit: submit
        }), recordSaveApi.save({
    ...
    $scope.state = null, $scope.initializeState = function() {
        ...
        confirmPromise.then(function() {
            ...
            return modals.open("post-save", {
                submit: submit
            }, !0);
        }).then($scope.logout, function() {
            saveSuccess && $scope.init();
        });
...
angular.module("essTime").directive("recordSavingModal", [ "appProps", "modals", savingModal ]).directive("recordPostSaveModal", [ "appProps", "modals", postSaveModal ]).directive("recordSubmitAckModal", [ "appProps", "modals", submitAckModal ]).directive("recordFutureEndConfModal", [ "appProps", "modals", futureEndDateConfModal ]).directive("recordExpectedHoursModal", [ "appProps", "modals", expectedHoursModal ]);
