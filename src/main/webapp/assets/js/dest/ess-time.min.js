...
function recordManageCtrl($scope, $q, $filter, appProps, recordUtils, modals, badgeService, supEmpGroupService, supRecordsApi, timeRecordReviewApi, reminderApi) {
    ...
    function getExtendedSupEntries() {
        var supEmpGroups = supEmpGroupService.getSupEmpGroupList(), supName = supEmpGroupService.getName(appProps.user.employeeId), extSupGroup = "Supervisors Under " + supName.fullName;
        return supEmpGroups.slice(1).filter(function(empGroup) {
            return moment().subtract(1, "year").isBefore(empGroup.effectiveTo);
        }).map(function(empGroup) {
            var supId = empGroup.supId, name = supEmpGroupService.getName(supId), baseLabel = getSupNameLabel(name);
            return {
                querySupId: supId,
                supId: supId,
                name: name,
        ...
        link: function($scope, $elem, $attrs) {
            ...
            function setSupGroupLabels() {
                ...
                angular.forEach($scope.supEmpGroups, function(empGroup) {
                    empGroup.supId === appProps.user.employeeId ? empGroup.dropDownLabel = supName : (empGroup.group = "Supervisors Under " + supName, 
            ...
            function employeeFilter(empInfo) {
                return activeFilter(empInfo) && senatorFilter(empInfo);
            }
...
function accrualProjectionDirective($timeout, appProps, AccrualHistoryApi, EmpInfoApi, modals, accrualUtils) {
    return {
        ...
        link: function($scope) {
            ...
            $scope.$watch("empId", getAccSummaries), $scope.isUser = function() {
                return $scope.empId === appProps.user.employeeId;
            }, $scope.isLoading = function() {
                for (var dataType in $scope.request) if ($scope.request.hasOwnProperty(dataType) && $scope.request[dataType]) return !0;
                return !1;
...
} ]), angular.module("essTime").filter("entryHours", [ entryHoursFilter ]).directive("timeRecordInput", [ timeRecordInputDirective ]).directive("recordDetails", [ "appProps", "modals", "AccrualPeriodApi", recordDetailsDirective ]).directive("recordDetailModal", [ "modals", recordDetailModalDirective ]);
...
essTime.directive("accrualProjections", [ "$timeout", "appProps", "AccrualHistoryApi", "EmpInfoApi", "modals", "AccrualUtils", accrualProjectionDirective ]), 
