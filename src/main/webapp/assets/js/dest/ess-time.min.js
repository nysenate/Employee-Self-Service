...
function accrualHistoryDirective($timeout, appProps, modals, AccrualHistoryApi, EmpInfoApi, ActiveYearsTimeRecordsApi) {
    return {
        ...
        link: function($scope, $elem, $attrs) {
            ...
            $scope.$watchCollection("empSupInfo", getEmpInfo), $scope.$watchCollection("empSupInfo", getEmpActiveYears), 
            $scope.$watchCollection("empSupInfo", clearAccSummaries), $scope.$watch("selectedYear", getAccSummaries), 
...
function accrualProjectionDirective($timeout, appProps, AccrualHistoryApi, EmpInfoApi, modals, accrualUtils) {
    return {
        ...
        link: function($scope, $elem, $attrs) {
            ...
            $scope.$watch("empId", getEmpInfo), $scope.$watch("empId", getAccSummaries), $scope.isUser = function() {
                return $scope.empId === appProps.user.employeeId;
            }, $scope.isLoading = function() {
                for (var dataType in $scope.request) if ($scope.request.hasOwnProperty(dataType) && $scope.request[dataType]) return !0;
                return !1;
