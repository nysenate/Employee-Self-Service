...
function accrualDetailDirective(appProps, modals, accrualUtils) {
    return {
        ...
        link: function($scope, $elem, $attrs) {
            $scope.accruals = modals.params().accruals, $scope.close = modals.resolve;
        }
    };
}
        ...
        link: function($scope, $elem, $attrs) {
            function getEmpInfo() {
                if ($scope.empId && $scope.isUser()) {
                    ...
                    }, $scope.handleErrorResponse).$promise.finally(function() {
            ...
            function getAccSummaries() {
                ...
                if (year && !$scope.accSummaries[year]) {
                    ...
                    $scope.request.accSummaries = !0, AccrualHistoryApi.get(params, function(resp) {
                        ...
                        $scope.handleErrorResponse(resp), $scope.error = {
    ...
    return {
        ...
        link: function($scope, $elem, $attrs) {
            function getAllowance() {
                function onSuccess(resp) {
                    if (1 !== (resp.result || []).length) return onFail(resp);
                ...
                return $scope.request.allowance = !0, allowanceApi.get(params, onSuccess, $scope.handleErrorResponse).$promise.finally(function() {
    ...
    return {
        ...
        link: function($scope, $elem, $attrs) {
            ...
            function loadAllowance() {
                function onSuccess(resp) {
                    if (1 !== (resp.result || []).length) return onFail(resp);
                    $scope.allowance = resp.result[0], allowanceUtils.computeRemaining($scope.allowance, record);
                }
                if ($scope.showAllowance) {
                    ...
                    return $scope.loadingAllowance = !0, allowanceApi.get(params, onSuccess, $scope.handleErrorResponse).$promise.finally(function() {
...
function recordEntryCtrl($scope, $rootScope, $filter, $q, appProps, activeRecordsApi, recordSaveApi, accrualPeriodApi, expectedHoursApi, allowanceApi, miscLeaveGrantApi, holidayApi, recordCreationApi, activeRow, recordUtils, locationService, modals, promiseUtils, allowanceUtils) {
    ...
    function saveRecord(submit) {
        ...
        recordSaveApi.save({
            ...
            Object.keys(errorMap).map(parseInt).filter(function(errorCode) {
                return modifiedErrorCodes.indexOf(errorCode) >= 0;
            }).length > 0 ? modals.open("record-modified-dialog").then($scope.init).catch($scope.handleErrorResponse) : $scope.handleErrorResponse(resp);
        }).$promise.finally(function() {
    ...
    function getSubmitDialogs() {
        var submitDialogs = [], prevUnsubmittedTe = getPrevUnsubmittedTe();
        return prevUnsubmittedTe && prevUnsubmittedTe.length > 0 && submitDialogs.push(function() {
            return modals.open("unsubmitted-te-warning", {
                records: prevUnsubmittedTe
            }, !0);
        }), $scope.expectedHoursEntered() || submitDialogs.push(function() {
    ...
    function getPrevUnsubmittedTe() {
        ...
        return $scope.state.records.filter(function(rec) {
            return "E" === rec.scope && moment(rec.beginDate).isBefore(currentRec.beginDate);
        });
    }
    ...
    $scope.state = null, $scope.initializeState = function() {
        ...
        confirmPromise.then(function() {
            ...
            saveSuccess && locationService.logout();
        });
        ...
        return $scope.state.request.expectedHrs = !0, expectedHoursApi.get(params, onSuccess, $scope.handleErrorResponse).$promise.finally(cleanUp);
        ...
        if ($scope.canCreateNextRecord()) {
            ...
            recordCreationApi.save(params, {}, function(response) {
                ...
                $scope.handleErrorResponse(errorResponse), $scope.state.request.records = !1;
            });
        }
        ...
        return function(miscLeave) {
            if (!1 === miscLeave.restricted) return !0;
        ...
        for (var iReq in $scope.state.request) if ($scope.state.request.hasOwnProperty(iReq) && !0 === $scope.state.request[iReq]) return !0;
        return !1;
    }, $scope.preValidation = function() {
        var record = $scope.getSelectedRecord();
        $scope.errorTypes.reset(), checkForPrevUnsubmittedRaSa(record);
    }, $scope.errorTypes = {
        raSa: {
        ...
        te: {
            workHours: function(entry) {
                ...
                $scope.getAvailableHours() < 0 && ($scope.errorTypes.te.notEnoughWorkHours = !0, 
    ...
    return {
        ...
        link: function($scope, $elem, $attrs) {
            ...
            $scope.showDetails = function(record) {
                ...
                for (var request in $scope.state.request) if ($scope.state.request.hasOwnProperty(request) && !0 === $scope.state.request[request]) return !0;
                return !1;
            }, $scope.isUser = function() {
...
function recordManageCtrl($scope, $q, $filter, appProps, recordUtils, modals, badgeService, supEmpGroupService, supRecordsApi, timeRecordReviewApi, reminderApi) {
    ...
    function getEmployeeActiveRecords(supEntry) {
        ...
        return $scope.state.request.records = !0, supRecordsApi.get(params, onSuccess, $scope.handleErrorResponse).$promise.finally(function() {
            $scope.state.request.records = !1;
        });
    }
    function submitRecords(records) {
        ...
        }).catch($scope.handleErrorResponse).finally(function() {
    ...
    function postReminders(timeRecords) {
        ...
        return modals.open("record-reminder-posting"), reminderApi.save(params, {}, function() {
            modals.rejectAll(), modals.open("record-reminder-posted", !0);
        }, function(response) {
            modals.rejectAll(), "EMPLOYEE_INACTIVE" === ((response || {}).data || {}).errorCode ? ($scope.state.inactiveEmps = response.data.errorData, 
            modals.open("inactive-employee-email")) : $scope.handleErrorResponse(response);
        }).$promise;
    }
    ...
    function getOverrideSupEntries() {
        var extSupEmpGroup = supEmpGroupService.getExtSupEmpGroup(), supOverrides = {};
    ...
    function getEmpOverrideEntries() {
        var extSupEmpGroup = supEmpGroupService.getExtSupEmpGroup(), empOverrides = {};
    ...
    function getExtendedSupEntries() {
        return supEmpGroupService.getSupEmpGroupList().slice(1).filter(function(empGroup) {
            return moment().subtract(1, "year").isBefore(empGroup.effectiveTo);
        }).map(function(empGroup) {
            var supId = empGroup.supId, name = supEmpGroupService.getName(supId), baseLabel = getSupNameLabel(name), supSupId = supEmpGroupService.getSupId(empGroup.supId);
            return {
                querySupId: supId,
                supId: supId,
                name: name,
                group: "Supervisors Under " + supEmpGroupService.getName(supSupId).fullName,
                baseLabel: baseLabel,
                dropDownLabel: baseLabel,
                extendedSup: !0,
        ...
        affectedSupervisors.forEach(function(supEntry) {
            ...
            angular.forEach(records, function(record) {
                ...
                record.totals = recordUtils.getRecordTotals(record), affectedSupervisors.filter(function(supEntry) {
                    ...
                    (supEntry.records[status] = supEntry.records[status] || []).push(record);
                })));
            });
        }), affectedSupervisors.forEach(function(supEntry) {
    ...
    function getRecords(supEntry, status) {
        return (supEntry.records || {})[status] || [];
    }
    ...
    $scope.state = {
        ...
        for (var p in $scope.state.selectedIndices[status]) if ($scope.state.selectedIndices[status].hasOwnProperty(p) && !0 === $scope.state.selectedIndices[status][p]) return !0;
        return !1;
    ...
    return {
        ...
        link: function($scope, $elem, $attrs) {
            ...
            $scope.records.forEach(function(record) {
                ...
                -1 === $scope.empIds.indexOf(empId) && $scope.empIds.push(empId), $scope.empIdRecordMap.hasOwnProperty(empId) || ($scope.empIdRecordMap[empId] = []), 
...
angular.module("essTime").service("AccrualUtils", [ "$httpParamSerializer", "appProps", function() {
...
} ]), angular.module("essTime").directive("allowanceBar", [ "appProps", "AllowanceUtils", allowanceBarDirective ]), 
...
essTime.controller("GrantPrivilegesCtrl", [ "$scope", "$http", "appProps", "SupervisorChainApi", "SupervisorGrantsApi", "SupervisorOverridesApi", "modals", function($scope, $http, appProps, SupervisorChainApi, SupervisorGrantsApi, SupervisorOverridesApi, modals) {
    $scope.state = {
        ...
        if (!0 === $scope.state.modified && !0 === $scope.state.fetched) {
            var modifiedGrantees = $scope.state.grantees.filter(function(grantee) {
                return !0 === grantee.modified;
...
} ]), angular.module("essTime").factory("supEmpGroupService", [ "$filter", "$q", "appProps", "modals", "RestErrorService", "SupervisorEmployeesApi", "SupervisorOverridesApi", function($filter, $q, appProps, modals, RestErrorService, supEmployeeApi, supOverrideApi) {
    function init() {
...
function() {
    ...
    angular.module("essTime").directive("recordPostSaveModal", [ "appProps", "modals", postSaveModal ]).directive("recordSubmitAckModal", [ "appProps", "modals", submitAckModal ]).directive("recordFutureEndConfModal", [ "appProps", "modals", futureEndDateConfModal ]).directive("recordExpectedHoursModal", [ "appProps", "modals", expectedHoursModal ]).directive("recordUnsubmittedTeModal", [ "appProps", "modals", recordUnsubmittedTeModal ]);
}();
...
function() {
    ...
    function initializeRandomBug() {
        return initializeBug(Math.random() * width, Math.random() * height);
    }
    ...
    function mouseOnBug(bug) {
        ...
        }, bugRadius = getDistance(bugRadiusVect);
        return getDistance(getMovementVector(bug, mouse)) < bugRadius + 50;
    }
    ...
    function updateBug(bug, modifier) {
        var movementVector = getMovementVector(mouse, bug), directionVector = getDirectionVector(movementVector);
        getDistance(movementVector) < bugConst.sight && moveSprite(bug, directionVector, bug.speed * modifier);
    }
    ...
    function main() {
        ...
        then = now, update(delta / 1e3), render(), requestAnimationFrame(main);
    }
    var canvas = document.getElementById("dbug-canvas");
    if (canvas) {
        var ctx = canvas.getContext("2d"), requestAnimationFrame = function() {
            var w = window;
            return w.requestAnimationFrame || w.webkitRequestAnimationFrame || w.msRequestAnimationFrame || w.mozRequestAnimationFrame;
        }(), width = document.body.clientWidth, height = document.body.clientHeight, then = Date.now(), sprites = {}, bugConst = {
            pfx: "bug",
            idNo: 0,
            img: "https://goo.gl/7p9rz3",
            scale: .1,
            speed: 500,
            sight: 300
        }, bugs = [];
