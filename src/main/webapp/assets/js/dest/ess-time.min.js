/*! ess - v - 2016-09-07 */function recordEntryCtrl($scope, $rootScope, $filter, $q, $timeout, appProps, activeRecordsApi, recordSaveApi, accrualPeriodApi, allowanceApi, miscLeaveGrantApi, holidayApi, activeRow, recordUtils, locationService, modals, promiseUtils) {
    ...
    function getAccrualTabIndexFn(propName, defaultIndex) {
        return function(entry) {
            return null != entry[propName] && $scope.getSelectedRecord().dirty ? 1 : entry.total < 7 && !$scope.isWeekend(entry.date) ? 1 : defaultIndex;
        };
    }
    ...
    function checkRaSaHourIncrements(hours) {
        return isNaN(hours) || hours % 1 % .5 === 0 ? !0 : ($scope.errorTypes.raSa.halfHourIncrements = !0, 
        !1);
    }
    function checkTeHourIncrements(hours) {
        return isNaN(hours) || hours % 1 % .25 === 0 ? !0 : ($scope.errorTypes.te.fifteenMinIncrements = !0, 
        !1);
    }
    $scope.state = null, $scope.initializeState = function() {
        ...
        return allowance && !isNaN(tempWorkHours) ? allowance.remainingHours - tempWorkHours : void 0;
        ...
        return $scope.state.annualEntries ? $scope.state.accrual.biWeekHrsExpected <= $scope.state.totals.raSaTotal : !0;
    }, $scope.futureEndDate = function() {
        var record = $scope.getSelectedRecord();
        return moment(record.endDate).isAfter(moment(), "day") ? !0 : void 0;
        ...
        raSa: {
            ...
            holidayHours: function(entry) {
                ...
                return 0 === hrs || null === hrs ? !0 : ("undefined" == typeof hrs && ($scope.errorTypes.raSa.holidayHoursInvalidRange = !0, 
                isValid = !1), isValid &= checkRaSaHourIncrements(hrs));
            },
            vacationHours: function(entry) {
                var hrs = entry.vacationHours, isValid = !0;
                return 0 === hrs || null === hrs ? !0 : ($scope.state.accrual && $scope.state.totals.vacationHours > $scope.state.accrual.vacationAvailable && ($scope.errorTypes.raSa.notEnoughVacationTime = !0, 
            ...
            personalHours: function(entry) {
                ...
                return 0 === hrs || null === hrs ? !0 : ($scope.state.accrual && $scope.state.totals.personalHours > $scope.state.accrual.personalAvailable && ($scope.errorTypes.raSa.notEnoughPersonalTime = !0, 
            ...
            sickEmpHours: function(entry) {
                ...
                return 0 === hrs || null === hrs ? !0 : (isValid &= isEnoughSickTime(), "undefined" == typeof hrs && ($scope.errorTypes.raSa.empSickHoursInvalidRange = !0, 
                isValid = !1), isValid &= checkRaSaHourIncrements(hrs));
            },
            sickFamHours: function(entry) {
                var hrs = entry.sickFamHours, isValid = !0;
                return 0 === hrs || null === hrs ? !0 : (isValid &= isEnoughSickTime(), "undefined" == typeof hrs && ($scope.errorTypes.raSa.famSickHoursInvalidRange = !0, 
                isValid = !1), isValid &= checkRaSaHourIncrements(hrs));
            },
            miscHours: function(entry) {
                var hrs = entry.miscHours, isValid = !0;
                return 0 === hrs || null === hrs ? !0 : ("undefined" == typeof hrs && ($scope.errorTypes.raSa.miscHoursInvalidRange = !0, 
                isValid = !1), isValid &= checkRaSaHourIncrements(hrs));
            },
            miscType: function(entry) {
                ...
                !1) : !0 : ($scope.errorTypes.raSa.noMiscTypeGiven = !0, !1);
            },
            totalHours: function(entry) {
                return isNaN(entry.total) || entry.total >= 0 && entry.total <= 24 ? !0 : ($scope.errorTypes.raSa.totalHoursInvalidRange = !0, 
                !1);
            }
        },
        te: {
    ...
    function initializeRecords(recordMap) {
        ...
        angular.forEach(recordMap, function(records, empId) {
            angular.forEach(records, function(record) {
                ...
                -1 == supIdList.indexOf(currSupId) && ($scope.state.supIds.push(currSupId), $scope.state.empInfos[currSupId] = record.supervisor);
        ...
        ActiveYearsTimeRecordsApi.get({
            ...
            if (resp.success) {
                ...
                $scope.state.recordYears = resp.years.filter(function(year) {
                    return year >= supStartYear && supEndYear >= year;
...
essTime.filter("timeRecordStatus", [ "$sce", function($sce) {
    ...
    return function(hours) {
        ...
        return hours > 0 ? (color = "#09BB05", sign = "+") : 0 > hours && (color = "#BB0505"), 
...
essApp.directive("supervisorRecordList", [ "appProps", "modals", function(appProps, modals) {
    function link($scope, $elem, $attr) {
        $scope.showDetails = function(record) {
            ...
            return $scope.selectedIndices ? $scope.setSelected(index, !$scope.selectedIndices[index]) : !1;
        }, $scope.setSelected = function(index, isSelected) {
            return $scope.selectedIndices ? isSelected ? ($scope.selectedIndices[index] = !0, 
            !0) : (delete $scope.selectedIndices[index], !1) : !1;
...
var essTime = angular.module("essTime"), binding = 0;
    ...
    return {
        ...
        link: function($scope, $elem, $attrs) {
            $scope.$watch("record.timeRecordId", function() {
                $timeout(function() {
                    var validatePending = !1;
                    $elem.children().each(function(index) {
                        function focusInCallBack(event) {
                            activeRow.setActiveRow(index), validate();
                        }
                        function focusOutCallback(event) {
                            activeRow.setActiveRow(null), validate();
                        }
                        function validate() {
                            validatePending = !0, $timeout(function() {
                                validatePending && ($scope.validateRecord(), $rootScope.$emit("validateRecordEntries"), 
                                validatePending = !1);
                            });
                        }
...
essTime.controller("PayPeriodCalendarCtrl", [ "$scope", "$http", "PayPeriodApi", "HolidayApi", "modals", function($scope, $http, PayPeriodApi, HolidayApi, modals) {
    $scope.state = {
        ...
        $scope.months = [];
        for (var i = 0; 12 > i; i++) $scope.months.push(moment().year(year).month(i).format("M/D/YYYY"));
