...
function empRecordHistoryCtrl($scope, $q, $timeout, appProps, ActiveYearsTimeRecordsApi, TimeRecordsApi, AttendanceRecordApi, SupervisorEmployeesApi, modals, recordUtils) {
    ...
    function getRecords() {
        ...
        if (!(iSelEmp < 0)) {
            ...
            if (!(year < 0)) {
                ...
                $q.all([ $scope.getTimesheetRecords(), $scope.getAttendRecords() ]).then(function() {
    ...
    function setActiveSupEmpGroup() {
        if (!($scope.state.iSelEmpGroup < 0)) {
            ...
            angular.forEach($scope.state.primaryEmps, function(emp) {
                emp.group = "Direct employees", setAdditionalEmpData(emp), empList.push(emp);
            }), isUser && (angular.forEach(selEmpGroup.empOverrideEmployees, function(emp) {
                emp.group = "Additional Employees", setAdditionalEmpData(emp, !0), empList.push(emp);
            }), angular.forEach(selEmpGroup.supOverrideEmployees.items, function(supGroup, supId) {
                angular.forEach(supGroup, function(emp) {
                    emp.group = $scope.state.nameMap[supId] ? $scope.state.nameMap[supId].lastName + "'s Employees" : "Sup Override Employees", 
                    setAdditionalEmpData(emp, !0), empList.push(emp);
                });
    ...
    function setAdditionalEmpData(emp, override) {
        var startDate = override ? emp.effectiveStartDate : emp.supStartDate, endDate = override ? emp.effectiveEndDate : emp.supEndDate, supStartMoment = moment(startDate || "1970-01-01"), supEndMoment = moment(endDate || "2999-12-31"), name = emp.empLastName + " " + emp.empFirstName[0] + ".", dateRange = supStartMoment.format("MMM YYYY") + (supStartMoment.isSame(supEndMoment, "month") ? "" : " - " + (supEndMoment.isBefore(moment(), "day") ? supEndMoment.format("MMM YYYY") : "Present"));
    ...
    function setNameMap() {
        ...
        }), allEmpInfos = primaryEmpInfos.concat(empOverrideInfos).concat(supOverrideInfos), empSupEmpGroupMap = extSupEmpGroup.employeeSupEmpGroups;
    ...
    function setSupEmpGroups() {
        var extSupEmpGroup = $scope.state.extSupEmpGroup, empSupEmpGroups = [], supName = appProps.user.firstName + " " + appProps.user.lastName;
        angular.forEach(extSupEmpGroup.employeeSupEmpGroups, function(supEmpGroups) {
            angular.forEach(supEmpGroups, function(empGroup) {
                ...
                setAdditionalEmpData(empGroup), empGroup.group = "Supervisors Under " + supName, 
                empSupEmpGroups.push(empGroup);
            });
        }), empSupEmpGroups.sort(function(a, b) {
            var aLabel = a.dropDownLabel, bLabel = b.dropDownLabel;
            return aLabel < bLabel ? -1 : aLabel > bLabel ? 1 : 0;
        }), extSupEmpGroup.dropDownLabel = supName, $scope.state.supEmpGroups = [ extSupEmpGroup ].concat(empSupEmpGroups);
    }
...
function accrualProjectionCtrl($scope, $timeout, appProps, AccrualHistoryApi, EmpInfoApi, modals, accrualUtils) {
    ...
    $scope.state = {
        ...
        for (var i = 0; i < projections.length; i++) {
            ...
            lastRec && accrualUtils.isFirstRecordOfYear(rec) && (multiYear = !0, rec.vacationBanked = Math.min(lastRec.vacationAvailable, maxVacationBanked), 
            rec.sickBanked = Math.min(lastRec.sickAvailable, maxSickBanked), per = vac = sickEmp = sickFam = 0), 
