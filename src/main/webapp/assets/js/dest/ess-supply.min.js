...
function supplyCookieService($cookies, appProps, lineItemService) {
    ...
    return {
        saveCartCookie: function(cart) {
            ...
            var code = encodeCart(serializatize(lineItems)), cur = $cookies.get(userId);
            null == cur && void 0 == cur || $cookies.remove(userId), $cookies.put(userId, code);
        },
        getCart: function() {
            var cur = $cookies.get(userId);
            if (null == cur || void 0 == cur) return [];
            var functionalLineItems = [], lineItems = deserialize(decodeCart(cur));
            return lineItems.forEach(function(lineItem) {
...
function supplyOrderController($scope, appProps, locationService, supplyCart, paginationModel, locationAutocompleteService, allowanceService, destinationService, modals, supplyUtils, lineItemService, itemFilterService) {
    ...
    function loadShoppingState() {
        ...
        allowanceService.queryLocationAllowance(destinationService.getDestination()).then(initializeCart).then(filterLineItems).then(setToShoppingState).then(setDestinationDescription).then($scope.updateSort).catch(loadItemsError);
    }
    ...
    $scope.init = function() {
        ...
        $scope.filter.searchTerm = "", filterLineItems();
    }, $scope.onPageChange = function() {
        ...
        $scope.state == $scope.states.SHOPPING && filterLineItems());
        ...
        $scope.sorting[$scope.sortBy] == $scope.sorting.Name ? lineItemsCopy.sort(function(a, b) {
            return a.item.description < b.item.description ? -1 : a.item.description > b.item.description ? 1 : 0;
        }) : $scope.sorting[$scope.sortBy] == $scope.sorting.Category && lineItemsCopy.sort(function(a, b) {
...
function quantitySelectorCtrl($scope, modals, supplyCart) {
    ...
    $scope.addToCart = function(lineItem) {
        supplyCart.isItemIdOrdered(lineItem.item.id) || "SPECIAL" !== lineItem.item.visibility ? (lineItem.increment(), 
        updateAndSaveCart(lineItem)) : modals.open("special-order-item-modal", {
            lineItem: lineItem
        }).then(function() {
...
function itemFilterService() {
    function filterLineItemsByCategories(lineItems, categories) {
        ...
        return angular.forEach(lineItems, function(lineItem) {
            categories.indexOf(lineItem.item.category.name) !== -1 && filtered.push(lineItem);
        }), filtered;
    }
...
function supplyCartController($scope, cookies, supplyCart, requisitionApi, destinationService, appProps, modals, locationService, supplyUtils) {
    ...
    $scope.init = function() {
        ...
        displayedLineItems = [], supplyCart.reset();
    }, $scope.closeModal = function() {
        modals.resolve();
...
essSupply.service("SupplyCartService", [ "SupplyLocationAllowanceService", "SupplyCookieService", function(allowanceService, cookies) {
    ...
    return {
        initializeCart: function(lineItems) {
            ...
            return lineItems && lineItems.forEach(addToCart), cookies.getCart().forEach(addToCart), 
            cart;
        },
        updateCartLineItem: function(lineItem) {
            var li = angular.copy(lineItem);
            return 0 === li.quantity ? cart.delete(li.item.id) : cart.set(li.item.id, li), cart;
        },
        getCart: function() {
            return cart;
        },
        ...
        reset: function() {
            cart = new Map(), cookies.saveCartCookie(cart);
        },
        save: function() {
            cookies.saveCartCookie(cart);
        }
    };
} ]), essSupply = angular.module("essSupply").controller("SupplyCartController", [ "$scope", "SupplyCookieService", "SupplyCartService", "SupplyRequisitionApi", "SupplyOrderDestinationService", "appProps", "modals", "LocationService", "SupplyUtils", supplyCartController ]);
