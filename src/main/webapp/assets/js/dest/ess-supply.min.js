...
function supplyNavigationController($scope, $location, appProps, locationService, categoryService, destinationService) {
    $scope.getInitializedCategories = function() {
        return categoryService.setSelectedCategories(locationService.getSearchParam("category") || []), 
        ...
        $scope.shouldDisplayCategoryFilter && categoryService.setSelectedCategories(locationService.getSearchParam("category") || []);
    }), $scope.onCategoryUpdated = function() {
        locationService.setSearchParam("category", categoryService.getSelectedCategoryNames(), !0, !1);
    }, $scope.clearSelections = function() {
        categoryService.clearSelections(), $scope.onCategoryUpdated();
    };
}
...
function supplyCategoryService(locationAllowanceService) {
    ...
    return {
        ...
        getSelectedCategories: function() {
            var selected = [];
            return angular.forEach(this.getCategories(), function(cat) {
                cat.selected === !0 && selected.push(cat);
            }), selected;
        },
        getSelectedCategoryNames: function() {
            var names = [];
            return this.getSelectedCategories().forEach(function(category) {
...
function supplyOrderController($scope, appProps, locationService, supplyCart, paginationModel, locationAutocompleteService, allowanceService, destinationService, modals, supplyUtils, lineItemService, itemFilterService, categoryService) {
    ...
    function loadShoppingState() {
        ...
        allowanceService.queryLocationAllowance(destinationService.getDestination()).then(initializeCart).then(sortAndFilterLineItems).then(setToShoppingState).then(setDestinationDescription).catch(loadItemsError);
    }
    ...
    $scope.init = function() {
        ...
        $scope.state == $scope.states.SHOPPING && sortAndFilterLineItems());
        ...
        function reset() {
            $scope.state = $scope.states.LOADING, supplyCart.reset(), destinationService.reset(), 
...
var essSupply = angular.module("essSupply").controller("SupplyOrderController", [ "$scope", "appProps", "LocationService", "SupplyCartService", "PaginationModel", "SupplyLocationAutocompleteService", "SupplyLocationAllowanceService", "SupplyOrderDestinationService", "modals", "SupplyUtils", "SupplyLineItemService", "SupplyItemFilterService", "SupplyCategoryService", supplyOrderController ]);
