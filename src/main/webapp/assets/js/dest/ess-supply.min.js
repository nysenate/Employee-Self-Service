...
function supplyHistoryController($scope, supplyIssuersApi, requisitionApi, locationService, locationApi, paginationModel, supplyUtils) {
...
function supplyFulfillmentController($scope, requisitionApi, supplyEmployeesApi, itemApi, modals, $interval, locationService, locationStatisticsService, supplyUtils) {
    ...
    function isOverPerOrderMax(requisition) {
        ...
        return angular.forEach(requisition.lineItems, function(lineItem) {
            lineItem.quantity > lineItem.item.perOrderAllowance && (isOverPerOrderMax = !0);
        }), isOverPerOrderMax;
    }
    function containsSpecialItems(requisition) {
        var containsSpecialItems = !1;
        return angular.forEach(requisition.lineItems, function(lineItem) {
            lineItem.item.visibility && (containsSpecialItems = !0);
...
function fulfillmentEditingModal($scope, appProps, modals, reqSaveApi, reqRejectApi, reqProcessApi, locationAutocompleteService, itemAutocompleteService) {
    ...
    $scope.init = function() {
        ...
        if (appProps.user.employeeId === $scope.originalRequisition.customer.employeeId) return void ($scope.selfApprove = !0);
        $scope.selfApprove = !1, $scope.processReq();
        ...
        $scope.editableRequisition.destination = loc ? loc : $scope.originalRequisition.destination, 
        $scope.onUpdate();
...
function supplyOrderController($scope, locationService, supplyCart, paginationModel, locationAutocompleteService, itemApi, destinationService, modals, lineItemService, itemFilterService, categoryService, stateService) {
    ...
    $scope.init = function() {
        ...
        $scope.state.isShopping() && (updateFiltersFromUrlParams(), sortAndFilterLineItems());
        ...
        function reset() {
            $scope.state.toLoading(), supplyCart.reset(), destinationService.reset(), locationService.go("/supply/order", !0);
        }
...
function largeItemImageModal() {
    return {
        template: '<div><modal-close-button></modal-close-button><img style="max-height:800px; display:block;"ng-src="{{imgUrl}}"err-src="{{errorUrl}}"></div>',
...
function cartService(storageService, lineItemService) {
    ...
    var KEY = "supply-cart", cart = void 0;
    return {
...
function supplyCartController($scope, storageService, supplyCart, requisitionApi, destinationService, appProps, modals, locationService, supplyUtils) {
    ...
    $scope.init = function() {
        ...
        function reset() {
            supplyCart.reset(), locationService.go("/supply/order", !1);
        }
...
function supplyViewController($scope, historyApi, locationService, $window, $timeout, supplyUtils) {
    ...
    $scope.requisitionResponse = {}, $scope.requisitionHistory = {
        ...
        highlightMenu(locationService.getSearchParam("fromPage"));
...
var essSupply = angular.module("essSupply").controller("SupplyNavigationController", [ "$scope", "LocationService", "SupplyCategoryService", "SupplyOrderPageStateService", supplyNavigationController ]), essSupply = angular.module("essSupply");
...
essSupply = angular.module("essSupply").controller("SupplyHistoryController", [ "$scope", "SupplyIssuersApi", "SupplyRequisitionApi", "LocationService", "LocationApi", "PaginationModel", "SupplyUtils", supplyHistoryController ]);
...
essSupply = angular.module("essSupply").controller("SupplyFulfillmentController", [ "$scope", "SupplyRequisitionApi", "SupplyEmployeesApi", "SupplyItemApi", "modals", "$interval", "LocationService", "SupplyLocationStatisticsService", "SupplyUtils", supplyFulfillmentController ]), 
essSupply = angular.module("essSupply").controller("SupplyReconciliationController", [ "$scope", "SupplyRequisitionApi", "LocationService", "$window", "$timeout", supplyReconciliationController ]);
...
} ]).controller("SupplyQuantitySelectorCtrl", [ "$scope", "modals", "SupplyCartService", quantitySelectorCtrl ]), essSupply = angular.module("essSupply");
...
supply.directive("largeItemImageModal", [ largeItemImageModal ]), supply.controller("LargeItemImageModalCtrl", [ "$scope", "modals", "appProps", function($scope, modals, appProps) {
    $scope.commodityCode = modals.params().commodityCode, $scope.imgUrl = appProps.imageUrl + "/" + $scope.commodityCode + "_800.jpg", 
