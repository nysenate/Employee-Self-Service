...
function supplyItemHistoryCtrl($scope, $q, supplyUtils, locationApi, itemHistoryFactory, itemApi, modals) {
    ...
    $scope.onFilterChange = function() {
        $scope.loading = !0, itemHistoryFactory.updateItemHistories($scope.filters.item.codeToId[$scope.filters.item.selected] || "All", $scope.filters.location.selected, moment($scope.filters.date.from, "MM/DD/YYYY").startOf("day").format(), moment($scope.filters.date.to, "MM/DD/YYYY").endOf("day").format()).then(function() {
            $scope.loading = !1;
...
function supplyItemHistoryFactory(requisitionApi) {
    function parseApiResponse(requisitionResponse) {
        function isItemSelectedInFilter(lineItem) {
            return "All" === params.itemId || params.itemId === lineItem.item.id;
        }
        function addToResults(itemHistory) {
            itemHistories.map.has(itemHistory.key()) ? itemHistories.map.get(itemHistory.key()).add(itemHistory.requisitions[0], itemHistory.quantity) : itemHistories.map.set(itemHistory.key(), itemHistory);
        }
        return requisitionResponse.result.forEach(function(requisition) {
            requisition.lineItems.forEach(function(lineItem) {
                isItemSelectedInFilter(lineItem) && lineItem.quantity > 0 && addToResults(new ItemHistory(lineItem.item.commodityCode, requisition.destination.locId, lineItem.quantity, requisition));
            });
    ...
    function reset() {
        itemHistories.value = [], itemHistories.map = new Map(), params = {
            itemId: "All",
            locId: "All",
            from: {},
            to: {}
    ...
    return {
        updateItemHistories: function(itemId, locId, from, to) {
            return reset(), params.itemId = itemId, params.locId = locId, params.from = from, 
            params.to = to, requisitionApi.get(apiParams()).$promise.then(parseApiResponse).then(sortItemHistories);
        },
...
function supplyItemHistoryPrintCtrl($scope, itemHistoryFactory, itemApi, $window, $timeout) {
    ...
    $scope.itemHistories = [], $scope.params = {}, $scope.commodityCode = "", $scope.getItemQuantity = function(requisition, commodityCode) {
        var qty = 0;
        return requisition.lineItems.forEach(function(li) {
            li.item.commodityCode === commodityCode && (qty = li.quantity);
        }), qty;
        ...
        var total = 0;
        return $scope.itemHistories.forEach(function(i) {
            total += i.quantity;
        }), total;
    }, function() {
        $scope.itemHistories = itemHistoryFactory.getItemHistories(), $scope.params = itemHistoryFactory.getParams(), 
        "All" === $scope.params.itemId ? $scope.commodityCode = "All" : itemApi.item($scope.params.itemId).then(function(item) {
...
function supplyOrderHistoryCtrl($scope, appProps, locationService, empInfoApi, orderHistoryApi, paginationModel, modals) {
    ...
    function getLoggedInEmployeeInfo() {
        ...
        return empInfoApi.get(params, function() {}, $scope.handleErrorResponse).$promise;
    }
...
function supplyHistoryController($scope, supplyIssuersApi, requisitionApi, itemApi, locationService, locationApi, paginationModel, supplyUtils, modals) {
    ...
    $scope.onFilterChange = function() {
        $scope.loading = !0, $scope.paginate.reset(), getUpdatedOrders().$promise.then(doneLoading).catch($scope.handleErrorResponse);
    }, $scope.onPageChange = function() {
        $scope.loading = !0, getUpdatedOrders().$promise.then(doneLoading).catch($scope.handleErrorResponse);
...
function supplyFulfillmentController($scope, requisitionApi, supplyEmployeesApi, itemApi, modals, $interval, locationService, locationStatisticsService, supplyUtils, $q) {
    ...
    function getApprovedShipments() {
        ...
        return $scope.approvedSearch.response = requisitionApi.get(params), $scope.approvedSearch.response.$promise.then(function(response) {
            $scope.approvedSearch.matches = response.result, sortRequisitionsByIdDesc($scope.approvedSearch.matches), 
            $scope.approvedSearch.error = !1;
        }).catch($scope.handleErrorResponse);
    }
    function sortRequisitionsByIdDesc(reqs) {
        return reqs.sort(function(a, b) {
            return a.requisitionId < b.requisitionId ? 1 : a.requisitionId > b.requisitionId ? -1 : 0;
        }), reqs;
    }
    ...
    function errorSaving(response) {
        if (void 0 !== response) return 409 === response.status ? $scope.saveResponse.error = !0 : $scope.handleErrorResponse(response), 
        response;
    }
...
function supplyReconciliationController($scope, requisitionApi, supplyReconciliationApi, locationService, supplyUtils, modals, $window, $timeout) {
        ...
        $scope.reconcilableSearch.response = requisitionApi.get(params, function(response) {
            ...
            angular.forEach($scope.reconcilableSearch.matches, function(shipment) {
                angular.forEach(shipment.lineItems, function(lineItem) {
                    ...
                    $scope.reconcilableItemMap[lineItem.item.id].push(shipment), lineItem.item.newQuantity = void 0, 
    ...
    $scope.selectedItem = null, $scope.viewItems = [], $scope.reconcilableSearch = {
        ...
        response: {},
        error: !1
    }, $scope.currentPage = 1, $scope.reconcilableItemMap = {}, $scope.placeHolderText = "Enter a Number", 
    $scope.print = function() {
        ...
        for (var i = 0; i < $scope.reconcilableSearch.items.length; i++) {
            ...
            order.itemId = $scope.reconcilableSearch.items[i].id, order.quantity = $scope.reconcilableSearch.items[i].newQuantity, 
...
function supplyViewController($scope, historyApi, locationService, $window, $timeout, supplyUtils, modals) {
    ...
    $scope.requisitionResponse = {}, $scope.requisitionHistory = {
        ...
        $scope.requisitionResponse = historyApi.get({
            id: id
        }), $scope.requisitionResponse.$promise.then(extractShipment).then(printIfRequested).then(generateHistory).then(selectCurrentVersion).catch($scope.handleErrorResponse);
    };
...
function supplyCartController($scope, storageService, supplyCart, requisitionApi, destinationService, appProps, modals, locationService, supplyUtils) {
    ...
    $scope.init = function() {
        ...
        null != savedDest && ($scope.destination = savedDest, $scope.destinationDescription = savedDest.locationDescription || ""), 
...
function supplyOrderController($scope, locationService, supplyCart, paginationModel, locationAutocompleteService, itemApi, destinationService, modals, lineItemService, itemFilterService, categoryService, stateService) {
    function loadSelectDestinationState() {
        locationAutocompleteService.initWithResponsibilityHeadLocations().then(destinationService.queryDefaultDestination).then(setDestinationCode).then(setToSelectingDestinationState).catch($scope.handleErrorResponse);
    }
...
angular.module("essSupply").factory("SupplyItemApi", [ "$resource", "appProps", "modals", "RestErrorService", function($resource, appProps, modals, RestErrorService) {
    function item(itemId) {
        return itemApi.get({
            itemId: itemId
        }).$promise.then(returnResult).catch(RestErrorService.handleErrorResponse);
    }
    function items() {
        return itemsApi.get().$promise.then(returnResult).catch(RestErrorService.handleErrorResponse);
    }
    ...
    var itemsApi = $resource(appProps.apiPath + "/supply/items.json"), itemApi = $resource(appProps.apiPath + "/supply/items/:itemId.json", {
        itemId: "@itemId"
    }), itemsForLocApi = $resource(appProps.apiPath + "/supply/items/orderable/:locId.json", {
        locId: "@locId"
    });
    return {
        item: item,
...
} ]), essSupply = angular.module("essSupply").controller("SupplyItemHistoryCtrl", [ "$scope", "$q", "SupplyUtils", "LocationApi", "SupplyItemHistoryFactory", "SupplyItemApi", "modals", supplyItemHistoryCtrl ]);

var essSupply = angular.module("essSupply");

essSupply.factory("SupplyItemHistoryFactory", [ "SupplyRequisitionApi", supplyItemHistoryFactory ]), 
essSupply = angular.module("essSupply").controller("SupplyItemHistoryPrintCtrl", [ "$scope", "SupplyItemHistoryFactory", "SupplyItemApi", "$window", "$timeout", supplyItemHistoryPrintCtrl ]);
...
essSupply = angular.module("essSupply").controller("SupplyReconciliationController", [ "$scope", "SupplyRequisitionApi", "SupplyReconciliationApi", "LocationService", "SupplyUtils", "modals", "$window", "$timeout", supplyReconciliationController ]);
