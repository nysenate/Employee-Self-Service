/*! ess - v - 2016-09-27 */function supplyNavigationController($scope, $location, appProps, locationService, categoryService, destinationService) {
...
function fulfillmentEditingModal($scope, appProps, modals, requisitionApi, locationAutocompleteService, itemAutocompleteService) {
    ...
    $scope.init = function() {
        ...
        return appProps.user.employeeId === $scope.originalRequisition.issuer.employeeId ? void ($scope.selfApprove = !0) : ($scope.selfApprove = !1, 
    ...
    }, $scope.warnning = !1, $scope.addItem = function() {
        var newItem = itemAutocompleteService.getItemFromCommodityCode($scope.newItemCommodityCode);
        return !newItem || isItemADuplicate(newItem) ? void ($scope.warnning = !0) : ($scope.warnning = !1, 
...
function supplyOrderController($scope, appProps, locationService, supplyCart, paginationModel, locationAutocompleteService, allowanceService, destinationService, modals, supplyUtils) {
    ...
    function loadShoppingState() {
        ...
        allowanceService.queryLocationAllowance(destinationService.getDestination()).then(saveAllowances).then(filterAllowances).then(setAllowances).then(setToShoppingState).then(setDestinationDescription).then(checkSortOrder);
    }
