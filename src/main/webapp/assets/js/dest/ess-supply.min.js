/*! ess - v - 2016-09-07 */function supplyNavigationController($scope, $location, appProps, locationService, categoryService, destinationService) {
    ...
    function updateSelectedCategoriesFromUrl() {
        ...
        angular.forEach(categoryService.getCategories(), function(category) {
            category.selected = -1 !== categoryNamesInUrl.indexOf(category.name);
        });
    }
    var categoryNamesInUrl = locationService.getSearchParam("category") || [];
    $scope.updateWithURL = function(e) {
        0 != categoryNamesInUrl.length && -1 != categoryNamesInUrl.indexOf(e.name) && (e.selected = !0);
...
function supplyHistoryController($scope, supplyEmployeesApi, requisitionApi, locationService, locationApi, paginationModel) {
    ...
    $scope.filter = {
        ...
        $scope.loading = !0, $scope.paginate.reset(), getUpdatedOrders().$promise.then(doneLoading);
    }, $scope.onPageChange = function() {
        $scope.loading = !0, getUpdatedOrders().$promise.then(doneLoading);
    };
    ...
    $scope.getOrderQuantity = function(shipment) {
        var size = 0;
        return angular.forEach(shipment.lineItems, function(lineItem) {
            size += lineItem.quantity;
        }), size;
    }, $scope.viewRequisition = function(shipment) {
        locationService.go("/supply/requisition/requisition-view", !1, "requisition=" + shipment.requisitionId);
    };
}
...
function supplyOrderHistoryCtrl($scope, appProps, locationService, empInfoApi, orderHistoryApi, paginationModel) {
    ...
    $scope.filter = {
        ...
        $scope.paginate.itemsPerPage = 12, $scope.filter.status = angular.copy($scope.STATUSES), 
        queryOrderHistory();
    }, $scope.init(), $scope.updateRequisitions = function() {
        $scope.loading = !0, queryOrderHistory();
    }, $scope.viewRequisition = function(requisition) {
        locationService.go("/supply/requisition/requisition-view", !1, "requisition=" + requisition.requisitionId);
    };
}

function supplyFulfillmentController($scope, requisitionApi, supplyEmployeesApi, itemsApi, modals, $interval, locationService, locationStatisticsService) {
    ...
    function getPendingShipments() {
        ...
        $scope.pendingSearch.response = requisitionApi.get(params, function(response) {
            ...
            $scope.pendingSearch.matches = [], $scope.pendingSearch.error = !0;
        });
    }
    function getProcessingShipments() {
        ...
        $scope.processingSearch.response = requisitionApi.get(params, function(response) {
            ...
            $scope.processingSearch.matches = [], $scope.processingSearch.error = !0;
        });
    }
    function getCompletedShipments() {
        ...
        $scope.completedSearch.response = requisitionApi.get(params, function(response) {
            ...
            $scope.completedSearch.matches = [], $scope.completedSearch.error = !0;
        });
    }
    function getApprovedShipments() {
        ...
        $scope.approvedSearch.response = requisitionApi.get(params, function(response) {
            ...
            $scope.approvedSearch.matches = [], $scope.approvedSearch.error = !0;
        });
    }
    function getCanceledShipments() {
        ...
        $scope.canceledSearch.response = requisitionApi.get(params, function(response) {
            ...
            $scope.canceledSearch.matches = [], $scope.canceledSearch.error = !0;
        });
    }
    ...
    $scope.$on("$destroy", function() {
        ...
        var size = 0;
        return angular.forEach(requisition.lineItems, function(item) {
            size++;
        }), size;
...
function fulfillmentEditingModal($scope, appProps, modals, requisitionApi, locationAutocompleteService, itemAutocompleteService) {
    ...
    $scope.init = function() {
        ...
        modals.resolve(requisitionApi.save({
        ...
        newItem && !isItemADuplicate(newItem) && ($scope.editableRequisition.lineItems.push({
            item: newItem,
            quantity: 1
        }), $scope.newItemCommodityCode = "", $scope.onUpdate());
...
function supplyOrderController($scope, appProps, locationService, supplyCart, paginationModel, locationAutocompleteService, allowanceService, destinationService, modals, supplyUtils) {
    function loadSelectDestinationState() {
        locationAutocompleteService.initWithResponsibilityHeadLocations().then(destinationService.queryDefaultDestination).then(setDestinationCode).then(setToSelectingDestinationState);
    }
    ...
    function loadShoppingState() {
        ...
        allowanceService.queryLocationAllowance(destinationService.getDestination()).then(saveAllowances).then(filterAllowances).then(setToShoppingState).then(setDestinationDescription).then(checkSortOrder);
    }
    ...
    $scope.init = function() {
        ...
        "more" === allowance.selectedQuantity && modals.open("order-more-prompt-modal", {
            allowance: allowance
        ...
        supplyCart.reset(), destinationService.reset(), locationService.go("/supply/order", !0);
...
function locationAllowanceService(allowanceApi) {
    function filterAllowancesByCategories(allowances, categories) {
        ...
        return angular.forEach(allowances, function(allowance) {
            -1 !== categories.indexOf(allowance.item.category.name) && filtered.push(allowance);
        }), filtered;
    }
    function filterAllowancesBySearch(allowances, searchTerm) {
        var filtered = [];
        return angular.forEach(allowances, function(allowance) {
            -1 !== allowance.item.description.indexOf(searchTerm.toUpperCase()) && filtered.push(allowance);
        }), filtered;
    }
    var allowances = null;
    return {
...
function orderDestinationService(appProps, cookies, empInfoApi, locationAutocompleteService) {
    ...
    return {
        queryDefaultDestination: function() {
            return defaultCode ? void 0 : empInfoApi.get({
                empId: appProps.user.employeeId,
                detail: !0
            }, function(response) {
...
function supplyCartController($scope, cookies, supplyCart, allowanceService, requisitionApi, destinationService, appProps, modals, locationService) {
    $scope.destinationCode = null, $scope.destinationDescription = "", $scope.init = function() {
        var destination = destinationService.getDestination();
        null != destination && ($scope.destinationCode = destination.code, $scope.destinationDescription = destinationService.getDestination().locationDescription || "");
        ...
        requisitionApi.save(params, function(response) {
            supplyCart.reset(), destinationService.reset(), cookies.resetDestination(), modals.open("supply-cart-checkout-modal", response);
        }, function(response) {});
...
function supplyViewController($scope, historyApi, locationService, $window, $timeout, supplyUtils) {
    ...
    $scope.init(), $scope.sortSelectedVersionLineItems = function() {
        return $scope.selectedVersion && $scope.selectedVersion.lineItems ? supplyUtils.alphabetizeLineItems($scope.selectedVersion.lineItems) : void 0;
...
essSupply = angular.module("essSupply").controller("SupplyHistoryController", [ "$scope", "SupplyEmployeesApi", "SupplyRequisitionApi", "LocationService", "LocationApi", "PaginationModel", supplyHistoryController ]);
...
essSupply = angular.module("essSupply").controller("SupplyFulfillmentController", [ "$scope", "SupplyRequisitionApi", "SupplyEmployeesApi", "SupplyItemsApi", "modals", "$interval", "LocationService", "SupplyLocationStatisticsService", supplyFulfillmentController ]), 
    ...
    return {
        ...
        link: function(scope, elm, attrs, ctrl) {
            ctrl.$validators.wholeNumber = function(modelValue, viewValue) {
                return modelValue % 1 === 0 && null !== modelValue;
            };
        }
    };
...
essSupply.service("SupplyCartService", [ "SupplyLocationAllowanceService", "SupplyCookieService", function(allowanceService, cookies) {
    ...
    return {
        isOverOrderAllowance: function(item, quantity) {
            return newQuantity(quantity, this.getCartLineItem(item.id)) > item.maxQtyPerOrder ? !0 : !1;
        },
