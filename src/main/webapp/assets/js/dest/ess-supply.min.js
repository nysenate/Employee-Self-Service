...
function supplyItemHistoryCtrl($scope, $q, supplyUtils, requisitionApi, locationApi, itemApi, modals) {
    ...
    function addToResults(lineItem, locCode) {
        var itemHistory = new ItemHistory(lineItem.item.commodityCode, locCode, lineItem.quantity);
        $scope.result.map.has(itemHistory.key()) ? $scope.result.map.get(itemHistory.key()).add(itemHistory.quantity) : $scope.result.map.set(itemHistory.key(), itemHistory);
    }
    ...
    var getRequisitions = function() {
        ...
        requisitionResponse.result.forEach(function(requisition) {
            requisition.lineItems.forEach(function(lineItem) {
                isItemSelectedInFilter(lineItem) && addToResults(lineItem, requisition.destination.locId);
            });
