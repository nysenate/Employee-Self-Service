    ...
    function pingServer() {
        $.ajax({
            ...
            success: function(data) {
                data.message > 0 && !1 === isTimeoutModalOpen && (modals.open("timeout"), isTimeoutModalOpen = !0, 
                $rootScope.$digest());
            },
            error: function(data) {
                (pingTolerance -= 1) < 0 && (window.location.replace(appProps.loginUrl), window.location.reload(!0));
            }
        });
    }
    var isPingInitialized = !1, isTimeoutModalOpen = !1, idleTime = 0, pingTolerance = 10;
    return {
        request: function(request) {
            if ("true" == globalProps.timeoutExempt) return request;
            if (!isPingInitialized) {
                var inactivityCheck = setInterval(function() {
                    idleTime += 30, pingServer();
                }, 3e4);
