...
function confirmModal(modals) {
    function link($scope, $elem, $attrs) {
        ...
        $scope.rejectButton = $attrs.rejectButton || "No", $scope.resolveClass = $attrs.resolveClass || "submit-button", 
        $scope.rejectClass = $attrs.rejectClass || "reject-button", $scope.confirmClass = $attrs.confirmClass || "time-neutral-button";
    }
    return {
        template: '<div class="confirm-modal {{level}}"><h3 class="content-info" ng-bind="title"></h3><div class="confirmation-message"><h4 ng-show="confirmMessage" ng-bind="confirmMessage"></h4><ng-transclude></ng-transclude><div ng-hide="rejectable" class="input-container"><input type="button" ng-click="resolve()" class="{{confirmClass}}" value="{{resolveButton}}" title="{{resolveButton}}" tabindex="1"/></div><div ng-show="rejectable" class="input-container"><input type="button" ng-click="resolve()" class="{{resolveClass}}" value="{{resolveButton}}" title="{{resolveButton}}" tabindex="1"/><input type="button" ng-click="reject()" class="{{rejectClass}}" value="{{rejectButton}}" title="{{rejectButton}}" tabindex="1"/></div></div></div>',
        transclude: !0,
        link: link
    };
}
...
function locationService($location, $window, $anchorScroll, $http, $route, appProps) {
    ...
    function logout(saveLocation) {
        ...
        saveLocation ? $http({
            ...
            go($location.path(), !0, $location.search());
        }) : go(logoutUrl, !0);
    }
    return {
...
essApi.factory("PayPeriodApi", [ "$resource", "appProps", function($resource, appProps) {
    ...
    return $resource(appProps.apiPath + "/travel/application/uncompleted/:empId.json", {
        empId: "@empId"
    });
} ]), essApi.factory("TravelApplicationPurposeApi", [ "$resource", "appProps", function($resource, appProps) {
    return $resource(appProps.apiPath + "/travel/application/uncompleted/:id/purpose.json", {
        id: "@id"
    }, {
        update: {
            method: "PUT"
    ...
    return $resource(appProps.apiPath + "/travel/application/uncompleted/:id/outbound.json", {
        id: "@id"
    }, {
        update: {
            method: "PUT"
    ...
    return $resource(appProps.apiPath + "/travel/application/uncompleted/:id/return.json", {
        id: "@id"
    }, {
        update: {
            method: "PUT"
    ...
    return $resource(appProps.apiPath + "/travel/application/uncompleted/:id/expenses.json", {
        id: "@id"
    }, {
        update: {
            method: "PUT"
    ...
    return $resource(appProps.apiPath + "/travel/application/uncompleted/:id/submit.json", {
        id: "@id"
    }, {
        update: {
            method: "PUT"
...
essApp.controller("MainCtrl", [ "$scope", "$http", "$route", "$routeParams", "$location", "$window", "appProps", "modals", "RestErrorService", "LocationService", function($scope, $http, $route, $routeParams, $location, $window, appProps, modals, RestErrorService, locationService) {
    ...
    }, $scope.logout = locationService.logout, $scope.log = function(stuff) {}, $scope.handleErrorResponse = RestErrorService.handleErrorResponse;
} ]);
        ...
        link: function($scope, $elem, $attrs) {
            function getSearchResults() {
                function onSuccess(resp) {
                    ...
                    }), pagination.setTotalItems(resp.total), validEmpId() && $scope.searchResults.length > 0 ? $scope.selectEmp($scope.searchResults[0]) : clearEmpId();
                }
                $scope.loadingEmps = !0;
                var params = {
            ...
            function clearEmpId() {
                empId = NaN, locationService.setSearchParam(EMP_ID_PARAM);
            }
            var EMP_ID_PARAM = "empId", TERM_PARAM = "term", ACTIVE_ONLY_PARAM = "activeOnly";
            $scope.selectedEmp = null, $scope.empInfo = null, $scope.activeOnly = "true" === locationService.getSearchParam(ACTIVE_ONLY_PARAM), 
            $scope.searchTerm = locationService.getSearchParam(TERM_PARAM) || "", $scope.searchResults = [];
            var empId = parseInt(locationService.getSearchParam(EMP_ID_PARAM) || NaN), pagination = angular.copy(paginationModel);
            pagination.itemsPerPage = 50, $scope.$watchGroup([ "searchTerm", "activeOnly" ], newSearch), 
            $scope.searchResultsExist = function() {
                ...
                $scope.selectedEmp = null, $scope.empInfo = null, empId && (clearEmpId(), newSearch()), 
                clearEmpId();
            };
        }
    };
...
} ]), angular.module("ess").service("LocationService", [ "$location", "$window", "$anchorScroll", "$http", "$route", "appProps", locationService ]), 
...
essApp.directive("textAutoHeight", [ "$timeout", function($timeout) {
    ...
    function onPingSuccess(resp) {
        ...
        remainingInactivity < 0 ? LocationService.logout(!0) : remainingInactivity <= warningThreshold && !modals.isOpen(timeoutModalName) && modals.open(timeoutModalName, {
            remainingInactivity: remainingInactivity
        }), active = !1;
    }
    function onPingFail(resp) {
        var errorCode = resp.status;
        failedPings++, (401 === errorCode || failedPings >= pingFailTolerance) && LocationService.logout(!0);
    }
