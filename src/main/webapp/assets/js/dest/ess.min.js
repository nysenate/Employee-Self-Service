...
function highlightFilter($sce, $sanitize) {
    var highlightedClass = "highlight-filter-highlighted", highlightTemplate = '<span class="' + highlightedClass + '">$1</span>';
    return function(text, term) {
        return term ? "string" != typeof text ? text : (text = $sanitize(text), text = text.replace(new RegExp("(" + term + ")", "gi"), highlightTemplate), 
...
essCore.constant("appProps", globalProps), essCore.run([ "$cookies", function($cookies) {
    ...
    $(document).unbind("keydown").bind("keydown", function(event) {
        if (8 === event.keyCode) {
            var doPrevent = !0, types = [ "text", "password", "file", "search", "email", "number", "date", "color", "datetime", "datetime-local", "month", "range", "search", "tel", "time", "url", "week" ], d = $(event.srcElement || event.target), disabled = d.prop("readonly") || d.prop("disabled");
            if (!disabled) if (d[0].isContentEditable) doPrevent = !1; else if (d.is("input")) {
...
essApp.controller("MainCtrl", [ "$scope", "$http", "$route", "$routeParams", "$location", "$window", "appProps", "modals", function($scope, $http, $route, $routeParams, $location, $window, modals, appProps) {
    ...
    var helpWindowName = "helpWindow", helpWindowOptions = "width=1024,height=768,location=no,menubar=no,personalbar=no,status=no,titlebar=no,toolbar=no";
    $scope.openHelpWindow = function($event) {
        $window.open($scope.helpPageUrl, helpWindowName, helpWindowOptions), $event.preventDefault();
...
}), angular.module("ess").filter("possessive", possessiveFilter), angular.module("ess").run([ "$rootScope", "$window", registerPrintHook ]), 
