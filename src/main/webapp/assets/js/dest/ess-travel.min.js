...
function travelAppController($scope, $q, appProps, modals, locationService, appInitApi, purposeApi, outboundApi, returnApi, expensesApi, travelApplicationApi, attachmentApi, motApi) {
    ...
    $scope.ACTIONS = {
        ...
        action === $scope.ACTIONS.NEXT ? ($scope.openLoadingModal(), purposeApi.update({
            ...
            updateAppFromResponse(response), updateStates(action);
        }).$promise.catch($scope.handleErrorResponse).finally($scope.closeLoadingModal)) : updateStates(action);
        ...
        action === $scope.ACTIONS.NEXT ? ($scope.openLoadingModal(), expensesApi.update({
            ...
            updateAppFromResponse(response), updateStates(action);
        }).$promise.then(updateAppFromResponse).catch($scope.handleErrorResponse).finally($scope.closeLoadingModal)) : updateStates(action);
...
essTravel.directive("travelApplicationPurpose", [ "appProps", "TravelApplicationAttachmentApi", "$http", function(appProps, attachmentApi, $http) {
    ...
    return {
        ...
        link: function($scope, $elem, $attrs, ctrl) {
            $scope.route = angular.copy($scope.app.route), 0 === $scope.route.outboundLegs.length && $scope.route.outboundLegs.push(new Segment()), 
    ...
    return {
        ...
        link: function($scope, $elem, $attrs, ctrl) {
            if ($scope.route = angular.copy($scope.app.route), 0 === $scope.route.returnLegs.length) {
                ...
                segment.to = $scope.app.route.outboundLegs[0].from, $scope.route.returnLegs.push(segment);
            }
            $scope.addSegment = function() {
                var segment = new Segment(), prevSeg = $scope.route.returnLegs[$scope.route.returnLegs.length - 1];
                segment.from = prevSeg.to, segment.modeOfTransportation = prevSeg.modeOfTransportation, 
                segment.isMileageRequested = prevSeg.isMileageRequested, segment.isMealsRequested = prevSeg.isMealsRequested, 
                segment.isLodgingRequested = prevSeg.isLodgingRequested, $scope.route.returnLegs.push(segment);
            }, $scope.isLastSegment = function(index) {
    ...
    return {
        ...
        link: function($scope, $elem, $attrs) {
            ...
            $scope.allowances = {
                ...
                for (var i = 0; i < $scope.route.outboundLegs.length; i++) if ("Personal Auto" === $scope.route.outboundLegs[i].modeOfTransportation) return !0;
                for (var y = 0; y < $scope.route.returnLegs.length; y++) if ("Personal Auto" === $scope.route.returnLegs[y].modeOfTransportation) return !0;
                return !1;
            }, $scope.isReimbursableLeg = function(leg) {
                return "Personal Auto" === leg.modeOfTransportation;
            };
        }
    };
