...
function addressCountyService($q, $timeout, modals, geocoder) {
    this.missingCounty = function(addresses) {
        ...
        var promises = [];
        return addresses.forEach(function(el, index, array) {
            var promise = geocoder.countyForAddress(el.formattedAddress).then(function(county) {
                el.county = county;
            });
        ...
        return function displayCountyModal() {
            var addr = addresses.shift();
            modals.isTop("address-county-modal") && modals.resolve(), void 0 == addr ? deferred.resolve() : (modals.isTop("loading") && modals.resolve(), 
            modals.open("address-county-modal", {
                address: addr
            }).then(function() {
                modals.open("loading"), $timeout(function() {
                    displayCountyModal();
                }, 200);
            }));
        }(), deferred.promise;
...
function addressCountyModalCtrl($scope, modals) {
    $scope.address = modals.params().address, $scope.resolveModal = function() {
        modals.resolve();
    };
}
...
function travelAppController($scope, $q, appProps, modals, locationService, appInitApi, purposeApi, outboundApi, returnApi, expensesApi, submitApi, motApi, cancelApi, geocoder) {
    function initApplication(travelerId) {
        ...
        appInitApi.save({
            ...
            $scope.app = response.result, hasUncompleteApplication() && modals.open("travel-continue-application-modal").catch(function() {
                $scope.openLoadingModal(), cancelApplication();
            });
    ...
    function updateStates(action) {
        action === $scope.ACTIONS.CANCEL ? function() {
            modals.open("cancel-application").then(function() {
                modals.resolve({}), $scope.openLoadingModal(), cancelApplication();
            });
    ...
    $scope.ACTIONS = {
        ...
        if (action === $scope.ACTIONS.NEXT) {
            ...
            for (var prop in allowances) allowances[prop] || (allowances[prop] = 0);
            expensesApi.update({
                id: $scope.app.id
            }, {
                ...
                updateAppFromResponse(response), updateStates(action), $scope.closeLoadingModal();
            }).$promise.catch(catchErrorResponse);
        } else updateStates(action);
...
function addressGeocoderService(geocodeApi, placeService) {
    this.countyForAddress = function(address) {
        return geocodeApi.get({
            address: address
        }).$promise.then(function(response) {
            return placeService.parseAddressFromPlace(response.results[0]).county;
        });
    };
}
...
function googlePlaceService() {
    this.parseAddressFromPlace = function(place) {
        ...
        var address = {};
        return address.formattedAddress = place.formatted_address, address.addr1 = function(place) {
            return getTypeName(place, "street_number") + " " + getTypeName(place, "route");
        }(place), address.city = function(place) {
            var city = getTypeName(place, "locality");
            return city || getTypeName(place, "administrative_area_level_3");
        }(place), address.county = function(place) {
            return getTypeName(place, "administrative_area_level_2");
        }(place), address.state = function(place) {
            return getTypeName(place, "administrative_area_level_1");
        }(place), address.zip5 = function(place) {
...
essTravel.directive("travelApplicationPurpose", [ "appProps", "$http", "TravelAttachmentDelete", function(appProps, $http, deleteAttachmentApi) {
            ...
            $scope.addSegment = function() {
                ...
                if ($scope.outboundForm.$valid) {
                    var addresses = $scope.route.outboundLegs.map(function(leg) {
                        ...
                        return leg.to;
                    })), addrsMissingCounty = countyService.missingCounty(addresses);
                    addrsMissingCounty.isEmpty ? $scope.continue() : ($scope.openLoadingModal(), countyService.updateWithGeocodeCounty(addrsMissingCounty).then(countyService.missingCounty).then(countyService.promptUserForCounty).then($scope.closeLoadingModal).then($scope.continue));
                }
    ...
    return {
        ...
        link: function($scope, $elem, $attrs, ctrl) {
            ...
            $scope.addSegment = function() {
                $scope.returnForm.$valid && ($scope.returnForm.$submitted = !1);
    ...
    return {
        ...
        link: function($scope, elm, attrs, ctrl) {
            ctrl.$validators.dateValidator = function(modelValue, viewValue) {
                return !!modelValue && (!!moment(modelValue, "MM/DD/YYYY", !0).isValid() || void 0);
            };
        }
    };
    ...
    return {
        ...
        link: function($scope, elm, attrs, ctrl) {
            elm.on("keydown", function(e) {
                ...
                address.addr1 = "", address.addr2 = "", address.city = "", address.county = "", 
                address.state = "", address.zip4 = "", address.zip5 = "";
            }), ctrl.$validators.addressValidator = function(modelValue, viewValue) {
                return !!modelValue && (!!$scope.leg[attrs.ngModel.split(".")[1]].addr1 || void 0);
            };
        }
    };
});
