...
function destSelectionCtrl($scope, modals) {
    ...
    $scope.destination = {
        ...
        modals.params().destination ? $scope.destination = modals.params().destination : modals.params().defaultModeOfTransportation && ($scope.destination.modeOfTransportation = modals.params().defaultModeOfTransportation);
    }, $scope.init();
}
...
function mileageDetailsModalCtrl($scope, modals) {
    ...
    $scope.displayRequirements = $scope.mileageAllowance.legs.length > 0 && 0 === Number($scope.mileageAllowance.total), 
    $scope.closeModal = function() {
        modals.resolve();
    };
}
    ...
    return {
        ...
        link: function($scope, $elem, $attrs) {
            function displayMap() {
                ...
                var destinations = $scope.reviewApp.destinations, origin = $scope.reviewApp.origin.formatted_address, waypoints = [];
                angular.forEach(destinations, function(dest, index) {
                    waypoints.push({
                        location: dest.address.formatted_address
                    });
                });
            ...
            $scope.reviewApp = angular.copy($scope.app), appApi.save({}, $scope.reviewApp, function(response) {
                modals.resolve({}), $scope.reviewApp = response.result, displayMap();
            }), modals.open("calculating-allowances-progress"), $scope.displayLodgingDetails = function() {
                modals.open("travel-lodging-details-modal", {}, !0);
            }, $scope.displayMealDetails = function() {
                modals.open("travel-meal-details-modal", {
                    app: $scope.reviewApp
                }, !0);
            }, $scope.displayMileageDetails = function() {
                modals.open("travel-mileage-details-modal", {}, !0);
            };
        }
    };
} ]);
    ...
    return {
        ...
        link: function($scope, $elem, $attrs) {
            ...
            autocomplete.addListener("place_changed", function() {
                ...
                address.formatted_address = place.formatted_address, address.addr1 = parseAddress1(place), 
                address.city = parseCity(place), address.state = parseState(place), address.zip5 = parseZip5(place), 
                $scope.callback({
                    address: address
                });
            }), $attrs.address && autocomplete.notify("place_changed", $attrs.address);
        }
    };
} ]);
