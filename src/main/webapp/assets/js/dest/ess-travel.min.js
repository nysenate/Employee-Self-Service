...
function destSelectionCtrl($scope, modals, modeOfTransportationApi) {
    ...
    $scope.modesOfTransportation = [], $scope.destination = {
        address: void 0,
        arrivalDate: void 0,
        ...
        $scope.destination.isMealsRequested = !0, "Personal Auto" === $scope.destination.modeOfTransportation && ($scope.destination.isMileageRequested = !0);
        var arrival = moment($scope.destination.arrivalDate, ISO_FORMAT), departure = moment($scope.destination.departureDate, ISO_FORMAT);
        Math.abs(arrival.diff(departure, "days")) > 0 && ($scope.destination.isLodgingRequested = !0), 
...
function travelAppController($scope, $q, appProps, modals, locationService, appInitApi, travelApplicationApi, attachmentApi, motApi) {
    ...
    function handleBackAction() {
        switch ($scope.appState) {
          ...
          case $scope.STATES.RETURN:
            $scope.appState = $scope.STATES.OUTBOUND, $scope.pageState = $scope.STATES.OUTBOUND;
            break;

          case $scope.STATES.ALLOWANCES:
            $scope.appState = $scope.STATES.RETURN, $scope.pageState = $scope.STATES.RETURN;
            break;
    ...
    $scope.ACTIONS = {
        ...
        REVIEW: 30,
        EDIT: 35
    }, $scope.pageState = void 0, $scope.appState = void 0, $scope.app = {}, function() {
        $scope.pageState = $scope.STATES.PURPOSE, $scope.appState = $scope.STATES.PURPOSE, 
        initApplication(appProps.user.employeeId), initModesOfTransportation();
        ...
        action === $scope.ACTIONS.NEXT && ($scope.app.outboundSegments = outboundSegments), 
        updateStates(action);
    }, $scope.returnCallback = function(returnSegments, action) {
        action === $scope.ACTIONS.NEXT && ($scope.app.returnSegments = returnSegments), 
        updateStates(action);
    }, $scope.destinationCallback = function(destinations, action) {
        action === $scope.ACTIONS.NEXT && $scope.setDestinations(destinations), updateStates(action);
        ...
        }).catch($scope.handleErrorResponse)), updateStates(action, editField);
    }, $scope.highlightOutboundStep = function() {
        return $scope.appState !== $scope.STATES.PURPOSE;
    }, $scope.highlightReturnStep = function() {
        return $scope.appState === $scope.STATES.RETURN || $scope.appState === $scope.STATES.ALLOWANCES || $scope.appState === $scope.STATES.REVIEW || $scope.appState === $scope.STATES.EDIT;
    }, $scope.highlightExpensesStep = function() {
        return $scope.appState === $scope.STATES.ALLOWANCES || $scope.appState === $scope.STATES.REVIEW || $scope.appState === $scope.STATES.EDIT;
    }, $scope.highlightReviewStep = function() {
        return $scope.appState === $scope.STATES.REVIEW || $scope.appState === $scope.STATES.EDIT;
    };
}

function Segment() {
    this.from = {}, this.to = {}, this.departureDate = "", this.arrivalDate = "", this.modeOfTransportation = {}, 
    this.isMileageRequested = !0, this.isMealsRequested = !0, this.isLodgingRequested = !0, 
...
function appPrintCtrl($scope, locationService, travelAppApi, motApi) {
    ...
    $scope.modeOfTransportations = [], $scope.app = {}, function() {
        var appId = locationService.getSearchParam("id");
        $scope.appRequest = travelAppApi.get({
            id: appId,
            detailed: !0
        }), $scope.appRequest.$promise.then(extractApplication).catch($scope.handleErrorResponse), 
        $scope.motRequest = motApi.get(), $scope.motRequest.$promise.then(extractMots).catch($scope.handleErrorResponse);
        ...
        return $scope.app.route.outgoingLegs.forEach(function(leg) {
            appModesOfTransportation.push(leg.modeOfTransportation);
        }), appModesOfTransportation.includes(mot);
    };
}
...
essTravel.controller("NewTravelApplicationCtrl", [ "$scope", "$q", "appProps", "modals", "LocationService", "TravelApplicationInitApi", "TravelApplicationApi", "TravelApplicationAttachmentApi", "TravelModeOfTransportationApi", travelAppController ]), 
essTravel.directive("travelApplicationPurpose", [ "appProps", "TravelApplicationAttachmentApi", "$http", function(appProps, attachmentApi, $http) {
    return {
        ...
        link: function($scope, $elem, $attrs) {
            $scope.purposeOfTravel = angular.copy($scope.app.purposeOfTravel), $scope.save = function(files) {
                for (var files = angular.element("#file")[0].files, formData = new FormData(), i = 0; i < files.length; i++) formData.append("file", files[i]);
    ...
    return {
        ...
        link: function($scope, $elem, $attrs, ctrl) {
            $scope.outboundSegments = angular.copy($scope.app.outboundSegments) || [], 0 === $scope.outboundSegments.length && $scope.outboundSegments.push(new Segment()), 
            $scope.addSegment = function() {
                var segment = new Segment(), prevSeg = $scope.outboundSegments[$scope.outboundSegments.length - 1];
                ...
                segment.isLodgingRequested = prevSeg.isLodgingRequested, $scope.outboundSegments.push(segment);
    ...
    return {
        ...
        scope: !0,
        link: function($scope, $elem, $attrs, ctrl) {
            if ($scope.returnSegments = angular.copy($scope.app.returnSegments) || [], 0 === $scope.returnSegments.length) {
                var segment = new Segment();
                segment.from = $scope.app.outboundSegments[$scope.app.outboundSegments.length - 1].to, 
                segment.to = $scope.app.outboundSegments[0].from, $scope.returnSegments.push(segment);
            }
            $scope.addSegment = function() {
                var segment = new Segment(), prevSeg = $scope.returnSegments[$scope.returnSegments.length - 1];
                segment.from = prevSeg.to, segment.modeOfTransportation = prevSeg.modeOfTransportation, 
                segment.isMileageRequested = prevSeg.isMileageRequested, segment.isMealsRequested = prevSeg.isMealsRequested, 
                segment.isLodgingRequested = prevSeg.isLodgingRequested, $scope.returnSegments.push(segment);
            }, $scope.isLastSegment = function(index) {
    ...
    return {
        ...
        link: function($scope, $elem, $attrs) {
            function displayMap() {
                ...
                var destinations = $scope.reviewApp.accommodations, origin = $scope.reviewApp.origin.formattedAddress, waypoints = [];
            ...
            }, $scope.handleErrorResponse), $scope.displayLodgingDetails = function() {
...
essTravel.controller("TravelApplicationPrintCtrl", [ "$scope", "LocationService", "TravelApplicationApi", "TravelModeOfTransportationApi", appPrintCtrl ]);
