...
function travelAppController($scope, $q, appProps, modals, locationService, appInitApi, purposeApi, outboundApi, returnApi, expensesApi, submitApi, motApi, cancelApi) {
    ...
    $scope.ACTIONS = {
        ...
        ALLOWANCES: 25,
        REVIEW: 30
    }, $scope.pageState = void 0, $scope.app = {}, function() {
        ...
        return $scope.pageState === $scope.STATES.RETURN || $scope.pageState === $scope.STATES.ALLOWANCES || $scope.pageState === $scope.STATES.REVIEW;
    }, $scope.highlightExpensesStep = function() {
        return $scope.pageState === $scope.STATES.ALLOWANCES || $scope.pageState === $scope.STATES.REVIEW;
...
function appPrintCtrl($scope, locationService, travelAppApi, motApi) {
    $scope.NOT_AVAILABLE = "N/A", $scope.modeOfTransportations = [], $scope.app = {}, 
    function() {
        ...
        travelAppApi.get({
            id: appId,
            detailed: !0
        }).$promise.then(extractApplication).catch($scope.handleErrorResponse), motApi.get().$promise.then(extractMots).catch($scope.handleErrorResponse);
...
function travelDetailCtrl($scope, modals) {
    $scope.app = modals.params(), $scope.getModesOfTransportation = function() {
        function arrayContains(array, obj) {
            var contains = !1;
            return array.forEach(function(element) {
                angular.equals(element, obj) && (contains = !0);
            }), contains;
        }
        var mots = [];
        return $scope.app.route.outboundLegs.concat($scope.app.route.returnLegs).forEach(function(leg, index) {
            var mot = leg.modeOfTransportation;
            arrayContains(mots, mot) || mots.push(mot);
        }), mots;
    }, $scope.exit = function() {
        modals.resolve();
    };
