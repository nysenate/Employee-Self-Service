...
essTravel.controller("ReviewHistoryCtrl", [
  "$scope",
  "LocationService",
  "modals",
...
function reviewHistory(
  ...
  (function () {
    appReviewApi
      .reviewHistory()
      .$promise.then(appReviewApi.parseAppReviewResponse)
    ...
    roleService.roles().then(function (response) {
  ...
  vm.applyFilters = function () {
    ...
    vm.data.reviews.filtered = vm.data.reviews.filtered.filter(function (r) {
      return (
        moment(r.travelApplication.activeAmendment.startDate, ISO_FORMAT) >=
          moment(vm.date.from, DATEPICKER_FORMAT) &&
        moment(r.travelApplication.activeAmendment.startDate, ISO_FORMAT) <=
          moment(vm.date.to, DATEPICKER_FORMAT)
      );
    });
    vm.data.reviews.filtered.sort(function (a, b) {
      return (
        moment(
          a.travelApplication.activeAmendment.startDate,
          ISO_FORMAT,
        ).format("x") -
        moment(
          b.travelApplication.activeAmendment.startDate,
          ISO_FORMAT,
        ).format("x")
      );
    });
